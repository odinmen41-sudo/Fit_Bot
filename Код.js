...
function doPost(e) {
  ...
  try {
    ...
    if (workoutHistory.handled) {
      ...
      logAiReply_(messageText, workoutHistory.message, "workout_logging_history");
      markBotInputProcessed_(inputRow, workoutHistory.ok ? "Да" : "Ошибка workout logging");
      return httpOk_("OK");
    }

    const coherentRecovery = routeRecoveryFacts_(update);
    if (coherentRecovery.handled) {
      sendTelegramMessage_(chatId, coherentRecovery.message);
      logAiReply_(messageText, coherentRecovery.message, "recovery_context");
      markBotInputProcessed_(inputRow, coherentRecovery.ok ? "Да" : "Ошибка recovery context");
      return httpOk_("OK");
    }

    const subjectiveRecovery = routeSubjectiveRecovery_(update);
    if (subjectiveRecovery.handled) {
      sendTelegramMessage_(chatId, subjectiveRecovery.message);
      logAiReply_(messageText, subjectiveRecovery.message, "subjective_recovery");
      markBotInputProcessed_(inputRow, subjectiveRecovery.ok ? "Да" : "Ошибка subjective recovery");
      return httpOk_("OK");
    }
    ...
    const weightTrends = routeWeightTrends_(update);
    if (weightTrends.handled) {
      sendTelegramMessage_(chatId, weightTrends.message);
      logAiReply_(messageText, weightTrends.message, "weight_trends");
      markBotInputProcessed_(inputRow, weightTrends.ok ? "Да" : "Ошибка weight trends");
      return httpOk_("OK");
    }

    const dailyDashboard = routeDailyDashboard_(update);
    if (dailyDashboard.handled) {
      sendTelegramMessage_(chatId, dailyDashboard.message);
      logAiReply_(messageText, dailyDashboard.message, "daily_dashboard");
      markBotInputProcessed_(inputRow, dailyDashboard.ok ? "Да" : "Ошибка daily dashboard");
      return httpOk_("OK");
    }
...
function handleDomainFactConfirmation_(selected, intent, userId, chatId, now, dependencies) {
  ...
  if (isB2NutritionCapture_(selected.payload) &&
    ...
    let primary = persisted && persisted.ok
    ...
    let coaching = null;
    if (persisted && persisted.ok === true && persisted.code === "NUTRITION_SAVED" && persisted.idempotent_replay !== true) {
      try {
        coaching = appendNutritionSuccessCoachingHint_(primary, userId, now, dependencies);
    ...
    return domainFactResult_(true, !!(persisted && persisted.ok),
      String(persisted && persisted.code || "NUTRITION_PERSISTENCE_FAILED"), {
        ...
        save: persisted || null,
        message: primary,
        coaching_hint: coaching && coaching.hint || null
      });
  }
...
function detectDailyNutritionQueryIntent_(text) {
  ...
  return /^(?:сколько\s+(?:я\s+)?(?:сегодня\s+)?съел(?:а)?|сколько\s+(?:я\s+)?съел(?:а)?\s+за\s+сегодня|сколько\s+(?:сегодня\s+)?калори(?:й|и)(?:\s+я\s+съел(?:а)?\s+сегодня)?|что\s+я\s+съел(?:а)?\s+сегодня|что\s+сегодня\s+по\s+кбжу)$/.test(normalized);
}
...
function detectWorkoutPlanIntent_(text) {
  ...
  if(/^(?:что\s+у\s+меня\s+сегодня\s+по\s+тренировке|какая\s+сегодня\s+тренировка|что\s+мне\s+сегодня\s+делать|что\s+сегодня\s+делать\s+в\s+зале|покажи\s+тренировку\s+на\s+сегодня|какие\s+сегодня\s+упражнения|есть\s+ли\s+сегодня\s+тренировка)$/.test(normalized))return {intent:"WORKOUT_TODAY"};
  return null;
}
...
/* WORKOUT LOGGING / HISTORY — authoritative append-only actual-performance model. */
const WORKOUT_LOG_LEGACY_SCHEMA = Object.freeze(["Дата","Тип тренировки","Упражнение","Вес","Подходы","Повторы","RPE","Боль/ограничения","Комментарий"]);
const WORKOUT_LOG_LIFECYCLE_COLUMNS = Object.freeze(["USER_ID","TELEGRAM_ID","SESSION_ID","SESSION_MODE","PLANNED_DATE","SESSION_KEY","SESSION_NAME","EVENT_TYPE","EVENT_ID","IDEMPOTENCY_KEY","EXERCISE_KEY","EXERCISE_NAME","SET_INDEX","ACTUAL_REPS","ACTUAL_LOAD","LOAD_UNIT","PERFORMED_AT","OPERATION_TYPE","LOGICAL_SET_ID","REPLACES_SET_ID","REVISION","SESSION_STATUS","COMPLETED_AT"]);
const WORKOUT_LOG_SCHEMA = Object.freeze(WORKOUT_LOG_LEGACY_SCHEMA.concat(WORKOUT_LOG_LIFECYCLE_COLUMNS));
const WORKOUT_CONFIRMATION_SOURCE = "WORKOUT_LOGGING_HISTORY";

function workoutActualResult_(handled,ok,code,extra){return Object.assign({handled:handled===true,ok:ok===true,code:String(code||""),groq_calls:0,production_writes:false},extra||{});}
function workoutActualText_(text){return String(text||"").toLowerCase().replace(/ё/g,"е").replace(/[–—]/g,"-").replace(/\s+/g," ").trim();}
function workoutActualId_(prefix,deps){return prefix+":"+(deps.uuid?deps.uuid():Utilities.getUuid());}
function workoutActualNumber_(value){const n=Number(String(value).replace(",","."));return isFinite(n)&&n>0?n:false;}
function workoutActualInteger_(value){const n=Number(value);return Number.isInteger(n)&&n>0?n:false;}
function workoutActualDate_(date,deps){return nutritionCalendarDate_(date,deps.time_zone(),0,deps.format_date);}

function detectWorkoutLoggingHistoryIntent_(text){
  const n=workoutActualText_(text);if(!n||/^\//.test(n))return null;
  if(/^(?:что у меня сегодня по тренировке|какая сегодня тренировка|что сегодня делать в зале|покажи тренировку на сегодня|какие сегодня упражнения|есть ли сегодня тренировка|какая|когда)\s*(?:следующая\s+)?тренировка$/.test(n))return null;
  if(/^(?:да|нет|отмена|отменить)$/.test(n))return {intent:/^да$/.test(n)?"CONFIRM":/^нет|отмен/.test(n)?"CANCEL":null};
  if(/^начал(?:а)? тренировку$/.test(n))return {intent:"START",session_hint:""};
  let m=n.match(/^начал(?:а)? (.+)$/);if(m)return {intent:"START",session_hint:m[1]};
  if(/^закончил(?:а)? тренировку$/.test(n))return {intent:"FINISH"};
  if(/^(?:что я сделал сегодня|покажи сегодняшнюю тренировку)\??$/.test(n))return {intent:"HISTORY_TODAY"};
  if(/^(?:покажи последнюю тренировку|что было на прошлой тренировке|что по тренировкам)\??$/.test(n))return {intent:"HISTORY_LAST"};
  m=n.match(/^что было на прошлой тренировке (.+?)\??$/);if(m)return {intent:"HISTORY_SESSION",session_text:m[1]};
  if(/^сколько тренировок было за (?:неделю|7 дней)\??$/.test(n))return {intent:"HISTORY_COUNT",days:7};
  m=n.match(/^(?:что я делал в прошлый|(?:какой вес|какие подходы) были? в прошлый раз в?) (.+?)\??$/);if(m)return {intent:"HISTORY_EXERCISE",exercise_text:m[1]};
  m=n.match(/^как прогрессирует (.+?)\??$/);if(m)return {intent:"PROGRESSION_EXERCISE",exercise_text:m[1]};
  if(/^какой вес был в прошлый раз\??$/.test(n))return {intent:"HISTORY_NEEDS_EXERCISE"};
...
function validateWorkoutLogSchema_(table){
  if(!table)return {ok:true,code:"NOT_CONFIGURED",headers:WORKOUT_LOG_SCHEMA.slice(),rows:[]};
  const headers=table.headers||[];for(let i=0;i<WORKOUT_LOG_LEGACY_SCHEMA.length;i+=1)if(headers[i]!==WORKOUT_LOG_LEGACY_SCHEMA[i])return {ok:false,code:"DATA_INTEGRITY_ERROR"};
  const indexes=workoutLogIndexes_(headers);for(let i=0;i<WORKOUT_LOG_LIFECYCLE_COLUMNS.length;i+=1)if(indexes[WORKOUT_LOG_LIFECYCLE_COLUMNS[i]]==null)return {ok:false,code:"SCHEMA_UPGRADE_REQUIRED"};
  return {ok:true,code:"SCHEMA_VALID",headers:headers,indexes:indexes,rows:table.rows||[]};
}
function workoutLogObject_(row,indexes){const object={};Object.keys(indexes).forEach(function(key){object[key]=row[indexes[key]];});return object;}
function workoutValidTimestamp_(value){const t=new Date(value).getTime();return isFinite(t)?new Date(t):null;}
function workoutPlannedDate_(value,deps){
  if(value==null||String(value).trim()==="")return "";
  const text=String(value).trim();if(/^\d{4}-\d{2}-\d{2}$/.test(text))return text;
  let date=value instanceof Date?value:null;if(!date&&typeof value==="number"&&value>1&&value<100000)date=new Date((value-25569)*86400000);if(!date)date=workoutValidTimestamp_(value);
  return date&&isFinite(date.getTime())?workoutActualDate_(date,deps):null;
}
function loadEffectiveWorkoutHistory_(telegramUserId,options){
  const runtime=options||{},deps=workoutActualDependencies_(runtime.dependencies),schema=validateWorkoutLogSchema_(deps.read_log());
  if(!schema.ok)return workoutActualResult_(true,false,schema.code,{sessions:[]});
  if(schema.code==="NOT_CONFIGURED")return workoutActualResult_(true,true,"NOT_FOUND",{sessions:[]});
  const expected=String(telegramUserId),events=[];
  for(let i=0;i<schema.rows.length;i+=1){const object=workoutLogObject_(schema.rows[i],schema.indexes),owner=String(object.TELEGRAM_ID||object.USER_ID||"");if(owner!==expected)continue;
    if(!object.SESSION_ID||!object.EVENT_TYPE||!object.EVENT_ID||!object.IDEMPOTENCY_KEY)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});events.push(object);}
  const sessions={},eventIds={};
  for(let i=0;i<events.length;i+=1){const e=events[i],sid=String(e.SESSION_ID),eid=String(e.EVENT_ID),plannedDate=workoutPlannedDate_(e.PLANNED_DATE,deps);if(eventIds[eid]||plannedDate===null)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});eventIds[eid]=true;
    if(!sessions[sid])sessions[sid]={session_id:sid,mode:String(e.SESSION_MODE||""),planned_date:plannedDate,session_key:String(e.SESSION_KEY||""),display_name:String(e.SESSION_NAME||"Тренировка"),status:"",started_at:null,completed_at:null,sets:[],skipped:[],events:[]};
    const s=sessions[sid],type=String(e.EVENT_TYPE);if(s.mode!==String(e.SESSION_MODE||"")||s.planned_date!==plannedDate||s.session_key!==String(e.SESSION_KEY||"")||s.display_name!==String(e.SESSION_NAME||"Тренировка"))return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});if(type!=="SESSION_START"&&!s.started_at||s.completed_at)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});sessions[sid].events.push(e);if(type==="SESSION_START"){const at=workoutValidTimestamp_(e.PERFORMED_AT);if(s.started_at||!at||!["PLAN_LINKED","AD_HOC"].includes(s.mode))return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});s.started_at=at;s.status="IN_PROGRESS";}
    else if(type==="SESSION_COMPLETE"){const at=workoutValidTimestamp_(e.COMPLETED_AT);if(!at||s.completed_at)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});s.completed_at=at;s.status="COMPLETED";}
  }
  const list=Object.keys(sessions).map(function(k){return sessions[k];});
  for(let a=0;a<list.length;a+=1){const s=list[a];if(!s.started_at)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});const logical={},skips={};
    for(let i=0;i<s.events.length;i+=1){const e=s.events[i],type=String(e.EVENT_TYPE);if(type==="SET"){const logicalId=String(e.LOGICAL_SET_ID||""),op=String(e.OPERATION_TYPE||"CREATE"),rev=Number(e.REVISION),reps=workoutActualInteger_(e.ACTUAL_REPS),load=workoutActualNumber_(e.ACTUAL_LOAD),idx=workoutActualInteger_(e.SET_INDEX),at=workoutValidTimestamp_(e.PERFORMED_AT);if(!logicalId||!["CREATE","CORRECT","DELETE"].includes(op)||!rev||!idx||!at)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});if(op!=="DELETE"&&(!reps||!load||String(e.LOAD_UNIT)!=="kg"||!e.EXERCISE_KEY||!e.EXERCISE_NAME))return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});const prior=logical[logicalId],graphInvalid=op==="CREATE"?(!!prior||rev!==1||String(e.REPLACES_SET_ID||"")!==""):(!prior||prior.deleted||rev!==prior.revision+1||String(e.REPLACES_SET_ID)!==String(prior.event_id));if(graphInvalid)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});logical[logicalId]={deleted:op==="DELETE",revision:rev,event_id:e.EVENT_ID,set_index:idx,exercise_key:String(e.EXERCISE_KEY),exercise_name:String(e.EXERCISE_NAME),reps:reps,load:load,load_unit:String(e.LOAD_UNIT),performed_at:at};}
      else if(type==="EXERCISE_SKIP"){if(!e.EXERCISE_KEY||!e.EXERCISE_NAME)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});skips[String(e.EXERCISE_KEY)]={exercise_key:String(e.EXERCISE_KEY),exercise_name:String(e.EXERCISE_NAME)};}}
    s.sets=Object.keys(logical).map(function(k){logical[k].logical_set_id=k;return logical[k];}).filter(function(set){return !set.deleted;}).sort(function(x,y){return x.set_index-y.set_index;});s.skipped=Object.keys(skips).map(function(k){return skips[k];});delete s.events;}
  list.sort(function(a,b){return a.started_at-b.started_at;});return workoutActualResult_(true,true,list.length?"HISTORY_LOADED":"NOT_FOUND",{sessions:list});
}

function workoutKnownExercises_(){return [{key:"barbell_row",name:"Тяга штанги в наклоне",aliases:["тяга штанги","тяга штанги в наклоне"]},{key:"squat",name:"Присед",aliases:["присед","приседания"]},{key:"bench_press",name:"Жим лёжа",aliases:["жим","жим лежа"]},{key:"deadlift",name:"Становая тяга",aliases:["становая","становая тяга"]},{key:"pullup",name:"Подтягивания",aliases:["подтягивания","подтягивание"]}];}
function resolveWorkoutExerciseIdentity_(text,planSession,activeExercise){const n=workoutActualText_(text);if(!n)return activeExercise||{ok:false,code:"EXERCISE_REQUIRED"};const plan=(planSession&&planSession.exercises||[]).filter(function(e){return workoutActualText_(e.display_name)===n||workoutActualText_(e.stable_exercise_identity)===n;});if(plan.length===1)return {ok:true,key:plan[0].stable_exercise_identity,name:plan[0].display_name,planned:true};if(plan.length>1)return {ok:false,code:"AMBIGUOUS_EXERCISE"};const known=workoutKnownExercises_().filter(function(e){return e.aliases.indexOf(n)>=0;});return known.length===1?{ok:true,key:known[0].key,name:known[0].name,planned:false}:{ok:false,code:known.length?"AMBIGUOUS_EXERCISE":"UNSUPPORTED_EXERCISE"};}
function workoutFindOpen_(history){return history.sessions.filter(function(s){return s.status==="IN_PROGRESS";});}
function workoutLatestExercise_(session){return session&&session.sets.length?{ok:true,key:session.sets[session.sets.length-1].exercise_key,name:session.sets[session.sets.length-1].exercise_name}:null;}
function workoutLifecycleRow_(event){const values={"Дата":event.local_date||"","Тип тренировки":event.session_name||"","Упражнение":event.exercise_name||"","Вес":event.load||"","Подходы":event.event_type==="SET"?1:"","Повторы":event.reps||"","RPE":"","Боль/ограничения":"","Комментарий":"Workout lifecycle","USER_ID":event.user_id,"TELEGRAM_ID":event.telegram_id,"SESSION_ID":event.session_id,"SESSION_MODE":event.session_mode,"PLANNED_DATE":event.planned_date||"","SESSION_KEY":event.session_key||"","SESSION_NAME":event.session_name,"EVENT_TYPE":event.event_type,"EVENT_ID":event.event_id,"IDEMPOTENCY_KEY":event.idempotency_key,"EXERCISE_KEY":event.exercise_key||"","EXERCISE_NAME":event.exercise_name||"","SET_INDEX":event.set_index||"","ACTUAL_REPS":event.reps||"","ACTUAL_LOAD":event.load||"","LOAD_UNIT":event.load_unit||"","PERFORMED_AT":event.performed_at||"","OPERATION_TYPE":event.operation_type||"","LOGICAL_SET_ID":event.logical_set_id||"","REPLACES_SET_ID":event.replaces_set_id||"","REVISION":event.revision||"","SESSION_STATUS":event.session_status||"","COMPLETED_AT":event.completed_at||""};return WORKOUT_LOG_SCHEMA.map(function(h){return values[h]==null?"":values[h];});}

function workoutActualDependencies_(overrides){const supplied=overrides||{};return {uuid:supplied.uuid||function(){return Utilities.getUuid();},time_zone:supplied.time_zone||function(){return Session.getScriptTimeZone();},format_date:supplied.format_date||function(d,z,p){return Utilities.formatDate(new Date(new Date(d).getTime()+Number(p||0)*86400000),z,"yyyy-MM-dd");},read_log:supplied.read_log||readWorkoutLifecycleTable_,append_atomic:supplied.append_atomic||appendWorkoutLifecycleRows_,load_plan:supplied.load_plan||function(uid,now){return loadAuthoritativeWorkoutPlan_(uid,{now:now});},create_capture:supplied.create_capture||createPendingCapture_,find_capture:supplied.find_capture||findWorkoutLoggingCapture_,cancel_capture:supplied.cancel_capture||cancelPendingCapture_,finalize_capture:supplied.finalize_capture||finalizeWorkoutCapture_};}
function readWorkoutLifecycleSheet_(sheet){if(!sheet)return null;const cols=Math.min(sheet.getLastColumn(),WORKOUT_LOG_SCHEMA.length),rows=sheet.getLastRow();if(!rows)return null;const values=sheet.getRange(1,1,rows,cols).getValues();return {headers:values[0],rows:values.slice(1)};}
function readWorkoutLifecycleTable_(){let spreadsheet;try{spreadsheet=getSpreadsheet_();}catch(error){return null;}return readWorkoutLifecycleSheet_(spreadsheet&&spreadsheet.getSheetByName("Workout_Log"));}
function workoutRowsPersisted_(table,rows){const schema=validateWorkoutLogSchema_(table);if(!schema.ok||schema.code==="NOT_CONFIGURED")return false;const eventIndex=schema.indexes.EVENT_ID,idempotencyIndex=schema.indexes.IDEMPOTENCY_KEY;if(eventIndex==null||idempotencyIndex==null)return false;return rows.every(function(expected){const eventId=String(expected[eventIndex]||""),key=String(expected[idempotencyIndex]||"");if(!eventId||!key)return false;return schema.rows.filter(function(actual){return String(actual[eventIndex]||"")===eventId&&String(actual[idempotencyIndex]||"")===key;}).length===1;});}
function appendWorkoutLifecycleRowsWithRuntime_(rows,runtime){const lock=runtime.lock();if(!lock.tryLock(5000))return {ok:false,code:"LOCK_TIMEOUT"};try{let spreadsheet,sheet;try{spreadsheet=runtime.get_spreadsheet();sheet=spreadsheet&&spreadsheet.getSheetByName("Workout_Log");}catch(error){return {ok:false,code:"PERSISTENCE_FAILED"};}if(!sheet)return {ok:false,code:"WORKOUT_LOG_NOT_CONFIGURED"};const before=readWorkoutLifecycleSheet_(sheet),schema=validateWorkoutLogSchema_(before);if(!schema.ok)return {ok:false,code:schema.code||"SCHEMA_UPGRADE_REQUIRED"};const keys=rows.map(function(r){return String(r[schema.indexes.IDEMPOTENCY_KEY]||"");});if(keys.some(function(k,i){return !k||keys.indexOf(k)!==i;}))return {ok:false,code:"INVALID_IDEMPOTENCY_BATCH"};const existing=before&&workoutRowsPersisted_(before,rows);if(existing)return {ok:true,code:"IDEMPOTENT_REPLAY",rows_written:0,idempotent_replay:true,verified:true};const existingKeys=(before&&before.rows||[]).map(function(r){return String(r[schema.indexes.IDEMPOTENCY_KEY]||"");});if(keys.some(function(k){return existingKeys.indexOf(k)>=0;}))return {ok:false,code:"DATA_INTEGRITY_ERROR"};try{if(rows.length)sheet.getRange(sheet.getLastRow()+1,1,rows.length,WORKOUT_LOG_SCHEMA.length).setValues(rows);runtime.flush();}catch(error){return {ok:false,code:"PERSISTENCE_FAILED"};}if(!workoutRowsPersisted_(readWorkoutLifecycleSheet_(sheet),rows))return {ok:false,code:"PERSISTENCE_READBACK_FAILED"};return {ok:true,code:"APPENDED",rows_written:rows.length,verified:true};}finally{lock.releaseLock();}}
function appendWorkoutLifecycleRows_(rows){return appendWorkoutLifecycleRowsWithRuntime_(rows,{lock:function(){return LockService.getScriptLock();},get_spreadsheet:function(){return getSpreadsheet_();},flush:function(){SpreadsheetApp.flush();}});}
function findWorkoutLoggingCapture_(userId,chatId,options){const now=options&&options.now||new Date();try{const rows=smartConfirmationReadRows_(smartConfirmationSheet_()).filter(function(row){const p=smartConfirmationParseJson_(row.payload_json,{});return p.source===WORKOUT_CONFIRMATION_SOURCE;});return selectActiveConfirmationCapture_(rows,userId,chatId,now,"NO_WORKOUT_CAPTURE");}catch(error){return {ok:false,code:"CAPTURE_LOOKUP_FAILED"};}}

function workoutPlanForToday_(userId,now,deps){const loaded=deps.load_plan(userId,now);if(!loaded||!loaded.ok||!loaded.configured)return null;const meta=workoutLocalDateMetadata_(now,0,{time_zone:deps.time_zone,format_date:deps.format_date});const session=loaded.sessions_by_weekday[meta.local_weekday];return session?{session:session,local_date:meta.local_date}:null;}
function workoutStartEvents_(userId,updateId,now,deps,hint,history){const open=workoutFindOpen_(history);if(open.length)return {ok:false,code:"UNFINISHED_SESSION",message:"У вас уже есть незавершённая тренировка."};const today=workoutPlanForToday_(userId,now,deps);let mode,sessionKey,sessionName,plannedDate;if(today&&(!hint||workoutActualText_(today.session.display_name)===workoutActualText_(hint))){mode="PLAN_LINKED";sessionKey=today.session.stable_session_identity;sessionName=today.session.display_name;plannedDate=today.local_date;}else if(hint&&["спина","ноги","грудь","Тяга штанги в наклоне","Присед","Жим лёжа","Становая тяга","Подтягивания"].map(workoutActualText_).indexOf(workoutActualText_(hint))>=0){mode="AD_HOC";sessionKey="";sessionName=String(hint).trim();plannedDate="";}else return {ok:false,code:"SESSION_CONTEXT_REQUIRED",message:"Сегодня нет однозначно определённой тренировки. Укажите поддерживаемое упражнение первым подходом."};const sid=workoutActualId_("workout",deps),eid=workoutActualId_("event",deps),when=now.toISOString(),local=workoutActualDate_(now,deps);return {ok:true,session:{session_id:sid,mode:mode,session_key:sessionKey,display_name:sessionName,planned_date:plannedDate,sets:[],skipped:[],status:"IN_PROGRESS",started_at:now},rows:[workoutLifecycleRow_({user_id:userId,telegram_id:userId,session_id:sid,session_mode:mode,planned_date:plannedDate,session_key:sessionKey,session_name:sessionName,event_type:"SESSION_START",event_id:eid,idempotency_key:"update:"+updateId,performed_at:when,session_status:"IN_PROGRESS",local_date:local})]};}
function workoutAppend_(deps,rows){let result;try{result=deps.append_atomic(rows);}catch(error){return {ok:false,code:"PERSISTENCE_FAILED"};}if(!result||result.ok!==true)return {ok:false,code:String(result&&result.code||"PERSISTENCE_FAILED")};if(result.verified===true)return result;let table;try{table=deps.read_log();}catch(error){return {ok:false,code:"PERSISTENCE_READBACK_FAILED"};}if(!workoutRowsPersisted_(table,rows))return {ok:false,code:"PERSISTENCE_READBACK_FAILED"};return Object.assign({},result,{ok:true,verified:true});}
function workoutIdempotencyApplied_(deps,updateId){const table=deps.read_log(),schema=validateWorkoutLogSchema_(table);if(!schema.ok||schema.code==="NOT_CONFIGURED")return false;const prefix="update:"+String(updateId);return schema.rows.some(function(row){const value=String(row[schema.indexes.IDEMPOTENCY_KEY]||"");return value===prefix||value.indexOf(prefix+":")===0;});}
function workoutTargetSet_(session,exerciseText,plan){const identity=resolveWorkoutExerciseIdentity_(exerciseText,plan,workoutLatestExercise_(session));if(!identity||!identity.ok)return {ok:false,code:identity&&identity.code||"EXERCISE_REQUIRED"};const matches=session.sets.filter(function(s){return s.exercise_key===identity.key;});return matches.length?{ok:true,set:matches[matches.length-1],identity:identity}:{ok:false,code:"SET_NOT_FOUND"};}
function createWorkoutPendingAction_(userId,chatId,updateId,now,deps,action,session,target,intent,exercise){const captureId="workout-action-"+String(updateId||deps.uuid()),payload={schema_version:"workout-logging-history-v1",source:WORKOUT_CONFIRMATION_SOURCE,capture_id:captureId,user_id:userId,chat_id:chatId,action:action,items:[{category:"WORKOUT_ACTION",fields:{action:{value:action,source:"EXPLICIT_USER_INPUT"}}}],frozen:{session_id:session.session_id,session_status:session.status,logical_set_id:target&&target.logical_set_id||"",effective_event_id:target&&target.event_id||"",revision:target&&target.revision||0,set_index:target&&target.set_index||0,exercise_key:exercise&&exercise.key||target&&target.exercise_key||"",exercise_name:exercise&&exercise.name||target&&target.exercise_name||"",old_reps:target&&target.reps||0,old_load:target&&target.load||0,new_reps:intent.reps?Number(intent.reps):0,new_load:intent.load?Number(String(intent.load).replace(",",".")):0}};const created=deps.create_capture(payload,{now:now,ttl_minutes:30,capture_id:captureId,user_id:userId,chat_id:chatId,source_update_id:updateId,validation:{ready_for_confirmation:true,errors:[]}});if(!created||!created.ok)return workoutActualResult_(true,false,created&&created.code||"CAPTURE_CREATE_FAILED",{message:"Не удалось подготовить подтверждение."});let message;if(action==="CORRECT")message="Исправить подход: "+target.load+" кг × "+target.reps+" → "+payload.frozen.new_load+" кг × "+payload.frozen.new_reps+"? Да / Нет";else if(action==="DELETE")message="Удалить последний подход "+target.exercise_name+" — "+target.load+" кг × "+target.reps+"? Да / Нет";else message="Отметить упражнение «"+exercise.name+"» как пропущенное? Да / Нет";return workoutActualResult_(true,true,"CONFIRMATION_REQUESTED",{message:message,pending_capture_writes:created.created===false?0:1});}
function finalizeWorkoutCapture_(capture,result,now){smartConfirmationUpdateState_(smartConfirmationSheet_(),capture.row_number,SMART_CONFIRMATION_CONFIG.STATUSES.SAVED,JSON.stringify(result),now,"");SpreadsheetApp.flush();}
function executeWorkoutPendingAction_(selected,userId,now,deps){const payload=selected.payload||{},f=payload.frozen||{},history=loadEffectiveWorkoutHistory_(userId,{dependencies:deps});if(!history.ok)return workoutActualResult_(true,false,history.code,{message:"История тренировок изменилась. Повторите действие."});const session=history.sessions.filter(function(s){return s.session_id===f.session_id&&s.status==="IN_PROGRESS";})[0];if(!session)return workoutActualResult_(true,false,"STALE_TARGET",{message:"Тренировка уже изменилась. Повторите действие."});let row,eventId=workoutActualId_("set",deps);if(payload.action==="SKIP"){row=workoutLifecycleRow_({user_id:userId,telegram_id:userId,session_id:session.session_id,session_mode:session.mode,planned_date:session.planned_date,session_key:session.session_key,session_name:session.display_name,event_type:"EXERCISE_SKIP",event_id:eventId,idempotency_key:"capture:"+payload.capture_id,exercise_key:f.exercise_key,exercise_name:f.exercise_name,performed_at:now.toISOString(),session_status:"IN_PROGRESS",local_date:workoutActualDate_(session.started_at,deps)});}else{const target=session.sets.filter(function(s){return s.logical_set_id===f.logical_set_id;})[0];if(!target||target.event_id!==f.effective_event_id||target.revision!==f.revision)return workoutActualResult_(true,false,"STALE_TARGET",{message:"Подход уже изменился. Повторите действие."});const deleting=payload.action==="DELETE";row=workoutLifecycleRow_({user_id:userId,telegram_id:userId,session_id:session.session_id,session_mode:session.mode,planned_date:session.planned_date,session_key:session.session_key,session_name:session.display_name,event_type:"SET",event_id:eventId,idempotency_key:"capture:"+payload.capture_id,exercise_key:target.exercise_key,exercise_name:target.exercise_name,set_index:target.set_index,reps:deleting?"":f.new_reps,load:deleting?"":f.new_load,load_unit:deleting?"":"kg",performed_at:now.toISOString(),operation_type:deleting?"DELETE":"CORRECT",logical_set_id:target.logical_set_id,replaces_set_id:target.event_id,revision:target.revision+1,session_status:"IN_PROGRESS",local_date:workoutActualDate_(session.started_at,deps)});}const written=workoutAppend_(deps,[row]);if(!written.ok)return workoutActualResult_(true,false,written.code,{message:"Не удалось сохранить изменение."});if(typeof deps.finalize_capture==="function")deps.finalize_capture(selected.capture,{action:payload.action,code:"SAVED"},now);return workoutActualResult_(true,true,payload.action+"_SAVED",{message:payload.action==="CORRECT"?"Подход исправлен.":payload.action==="DELETE"?"Подход удалён.":"Упражнение отмечено как пропущенное."});}
function executeWorkoutIntent_(userId,intent,updateId,now,options){const deps=workoutActualDependencies_(options&&options.dependencies),history=loadEffectiveWorkoutHistory_(userId,{dependencies:deps});if(!history.ok&&history.code!=="NOT_FOUND")return workoutActualResult_(true,false,history.code,{message:"История тренировок содержит ошибку данных."});history.sessions=history.sessions||[];
  if(workoutIdempotencyApplied_(deps,updateId))return workoutActualResult_(true,true,"IDEMPOTENT_REPLAY",{idempotent_replay:true,message:intent.intent==="FINISH"?"Тренировка уже завершена.":intent.intent==="START"?"Тренировка уже начата.":"Подход уже записан."});
  if(intent.intent==="START"){const built=workoutStartEvents_(userId,updateId,now,deps,intent.session_hint,history);if(!built.ok)return workoutActualResult_(true,false,built.code,{message:built.message});const w=workoutAppend_(deps,built.rows);return workoutActualResult_(true,!!w.ok,w.ok?"SESSION_STARTED":w.code,{message:w.ok?"Тренировка начата.":"Не удалось начать тренировку."});}
  const open=workoutFindOpen_(history);if(open.length>1)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{message:"Найдены конфликтующие незавершённые тренировки."});let session=open[0]||null;
  if(intent.intent==="FINISH"){if(!session)return workoutActualResult_(true,false,"NO_ACTIVE_SESSION",{message:"Нет активной тренировки."});const eid=workoutActualId_("event",deps),row=workoutLifecycleRow_({user_id:userId,telegram_id:userId,session_id:session.session_id,session_mode:session.mode,planned_date:session.planned_date,session_key:session.session_key,session_name:session.display_name,event_type:"SESSION_COMPLETE",event_id:eid,idempotency_key:"update:"+updateId,performed_at:now.toISOString(),session_status:"COMPLETED",completed_at:now.toISOString(),local_date:workoutActualDate_(session.started_at,deps)}),w=workoutAppend_(deps,[row]);return workoutActualResult_(true,!!w.ok,w.ok?"SESSION_COMPLETED":w.code,{message:w.ok?formatWorkoutCompletion_(session):"Не удалось завершить тренировку."});}
  if(intent.intent==="SETS"){const count=workoutActualInteger_(intent.count),reps=workoutActualInteger_(intent.reps),load=workoutActualNumber_(intent.load);if(!count||count>20||!reps||!load||intent.unit!=="kg")return workoutActualResult_(true,false,"INVALID_SET",{message:"Проверьте количество подходов, повторов и вес в кг."});let startRows=[];if(!session){const today=workoutPlanForToday_(userId,now,deps),identity=resolveWorkoutExerciseIdentity_(intent.exercise_text,today&&today.session,null);if(!identity.ok)return workoutActualResult_(true,false,identity.code,{message:"Укажите поддерживаемое упражнение точнее."});const started=workoutStartEvents_(userId,updateId+":start",now,deps,today?today.session.display_name:identity.name,history);if(!started.ok)return workoutActualResult_(true,false,started.code,{message:started.message});startRows=started.rows;session=started.session;}
    const today=workoutPlanForToday_(userId,now,deps),active=workoutLatestExercise_(session),identity=resolveWorkoutExerciseIdentity_(intent.exercise_text,today&&today.session,active);if(!identity||!identity.ok)return workoutActualResult_(true,false,identity&&identity.code||"EXERCISE_REQUIRED",{message:"Укажите упражнение точнее."});const existing=session.sets.filter(function(s){return s.exercise_key===identity.key;}),rows=startRows.slice();for(let i=0;i<count;i+=1){const eid=workoutActualId_("set",deps);rows.push(workoutLifecycleRow_({user_id:userId,telegram_id:userId,session_id:session.session_id,session_mode:session.mode,planned_date:session.planned_date,session_key:session.session_key,session_name:session.display_name,event_type:"SET",event_id:eid,idempotency_key:"update:"+updateId+":"+i,exercise_key:identity.key,exercise_name:identity.name,set_index:existing.length+i+1,reps:reps,load:load,load_unit:"kg",performed_at:now.toISOString(),operation_type:"CREATE",logical_set_id:eid,revision:1,session_status:"IN_PROGRESS",local_date:workoutActualDate_(session.started_at||now,deps)}));}const w=workoutAppend_(deps,rows);return workoutActualResult_(true,!!w.ok,w.ok?"SETS_LOGGED":w.code,{message:w.ok?(count===1?"Записал: "+identity.name+" — "+load+" кг × "+reps+".":"Записал: "+count+" подхода, "+identity.name+" — "+load+" кг × "+reps+"."):"Не удалось записать подход."});}
  if(["CORRECT","DELETE","SKIP"].indexOf(intent.intent)>=0){if(!session)return workoutActualResult_(true,false,"NO_ACTIVE_SESSION",{message:"Нет активной тренировки."});const today=workoutPlanForToday_(userId,now,deps),chatId=String(options&&options.chat_id||userId);if(intent.intent==="SKIP"){if(session.mode!=="PLAN_LINKED"||!today)return workoutActualResult_(true,false,"SKIP_REQUIRES_PLAN",{message:"Пропуск можно отметить только для упражнения текущего плана."});const exercise=resolveWorkoutExerciseIdentity_(intent.exercise_text,today.session,null);if(!exercise.ok||!exercise.planned)return workoutActualResult_(true,false,exercise.code||"UNSUPPORTED_EXERCISE",{message:"Уточните упражнение из текущего плана."});return createWorkoutPendingAction_(userId,chatId,updateId,now,deps,"SKIP",session,null,intent,exercise);}const target=workoutTargetSet_(session,intent.exercise_text,today&&today.session);if(!target.ok)return workoutActualResult_(true,false,target.code,{message:"Не удалось однозначно найти подход."});if(intent.intent==="CORRECT"&&(!workoutActualInteger_(intent.reps)||!workoutActualNumber_(intent.load)||intent.unit!=="kg"))return workoutActualResult_(true,false,"INVALID_SET",{message:"Проверьте повторения и вес в кг."});return createWorkoutPendingAction_(userId,chatId,updateId,now,deps,intent.intent,target.set?session:null,target.set,intent,target.identity);}
  return workoutActualResult_(false,true,"NOT_WORKOUT_MUTATION");}

function formatWorkoutCompletion_(session){const names=[],seen={};session.sets.forEach(function(s){if(!seen[s.exercise_name]){seen[s.exercise_name]=true;names.push(s.exercise_name);}});return "Тренировка завершена: "+session.display_name+". Выполнено подходов: "+session.sets.length+(names.length?". Упражнения: "+names.join(", "):"")+(session.skipped.length?". Пропущено: "+session.skipped.map(function(s){return s.exercise_name;}).join(", "):"")+".";}
function workoutHistoryIntentResult_(userId,intent,now,options){const deps=workoutActualDependencies_(options&&options.dependencies),history=loadEffectiveWorkoutHistory_(userId,{dependencies:deps});if(!history.ok)return workoutActualResult_(true,false,history.code,{message:"История тренировок содержит ошибку данных."});let sessions=history.sessions||[],selected=null;
  if(intent.intent==="HISTORY_NEEDS_EXERCISE")return workoutActualResult_(true,true,"CLARIFICATION_REQUIRED",{message:"Уточните упражнение, например: «какой вес был в прошлый жим»."});
  if(intent.intent==="HISTORY_COUNT"){const start=weightTrendShiftDate_(now,-6,deps),end=workoutActualDate_(now,deps),completed=sessions.filter(function(s){const date=workoutActualDate_(s.started_at,deps);return s.status==="COMPLETED"&&date>=start&&date<=end;});return workoutActualResult_(true,true,"HISTORY_COUNT",{message:"За последние 7 дней: "+completed.length+" завершённых тренировок."});}
  if(intent.intent==="HISTORY_SESSION"){const target=workoutActualText_(intent.session_text),completed=sessions.filter(function(s){return s.status==="COMPLETED"&&workoutActualText_(s.display_name).indexOf(target)>=0;}).sort(function(a,b){return b.completed_at-a.completed_at;});return workoutActualResult_(true,true,completed.length?"HISTORY_SESSION":"NOT_FOUND",{message:completed.length?formatWorkoutHistorySession_(completed[0]):"Завершённых тренировок с таким названием пока нет."});}
  if(intent.intent==="HISTORY_TODAY"){const date=workoutActualDate_(now,deps);const matches=sessions.filter(function(s){return workoutActualDate_(s.started_at,deps)===date;});return workoutActualResult_(true,true,matches.length?"HISTORY_TODAY":"NOT_FOUND",{message:matches.length?matches.map(formatWorkoutHistorySession_).join("\n\n"):"Сегодня фактических тренировок нет."});}
  if(intent.intent==="HISTORY_LAST"){const completed=sessions.filter(function(s){return s.status==="COMPLETED";}).sort(function(a,b){return b.completed_at-a.completed_at;});if(completed.length>1&&completed[0].completed_at.getTime()===completed[1].completed_at.getTime())return workoutActualResult_(true,false,"AMBIGUOUS_HISTORY",{message:"Не удалось однозначно определить последнюю тренировку."});selected=completed[0];return workoutActualResult_(true,true,selected?"HISTORY_LAST":"NOT_FOUND",{message:selected?formatWorkoutHistorySession_(selected):"Завершённых тренировок пока нет."});}
  const completed=sessions.filter(function(s){return s.status==="COMPLETED";}).sort(function(a,b){return b.completed_at-a.completed_at;}),identity=resolveWorkoutExerciseIdentity_(intent.exercise_text,null,null);if(!identity.ok)return workoutActualResult_(true,false,identity.code,{message:"Уточните упражнение."});selected=completed.filter(function(s){return s.sets.some(function(set){return set.exercise_key===identity.key;});})[0];if(!selected)return workoutActualResult_(true,true,"NOT_FOUND",{message:"Предыдущих выполненных подходов для этого упражнения нет."});const sets=selected.sets.filter(function(set){return set.exercise_key===identity.key;}),summary=identity.name+" в прошлый раз:\n"+sets.map(function(s,i){return (i+1)+". "+s.load+" кг × "+s.reps;}).join("\n");if(intent.intent!=="PROGRESSION_EXERCISE")return workoutActualResult_(true,true,"HISTORY_EXERCISE",{message:summary});const sameLoad=sets.length&&sets.every(function(s){return s.load===sets[0].load;});if(!sameLoad)return workoutActualResult_(true,true,"PROGRESSION_INSUFFICIENT",{message:summary+"\nДля однозначной прогрессии пока недостаточно сопоставимых подходов."});const target=Math.max.apply(null,sets.map(function(s){return s.reps;})),next=sets.map(function(s){return Math.min(target,s.reps+1);});return workoutActualResult_(true,true,"PROGRESSION_GUIDANCE",{message:summary+"\nМожно оставить "+sets[0].load+" кг и попробовать добрать "+next.join("/")+". Если есть боль или самочувствие хуже обычного, не форсируй прогрессию."});}
function formatWorkoutHistorySession_(session){const date=session.planned_date||session.started_at.toISOString().slice(0,10),lines=[date+" — "+session.display_name+(session.status==="IN_PROGRESS"?" (не завершена)":"")];session.sets.forEach(function(s){lines.push(s.exercise_name+": "+s.load+" кг × "+s.reps);});session.skipped.forEach(function(s){lines.push("Пропущено: "+s.exercise_name);});if(!session.sets.length&&!session.skipped.length)lines.push("Подходов пока нет.");return lines.join("\n");}

function routeWorkoutLoggingHistory_(update,options){const message=update&&(update.message||update.edited_message),intent=message&&typeof message.text==="string"?detectWorkoutLoggingHistoryIntent_(message.text):null;if(!intent)return workoutActualResult_(false,true,"NOT_WORKOUT_LOGGING_HISTORY");const userId=String(message.from&&message.from.id||"").trim(),chatId=String(message.chat&&message.chat.id||"").trim(),now=options&&options.now instanceof Date?options.now:new Date(),deps=workoutActualDependencies_(options&&options.dependencies);if(!userId||!chatId)return workoutActualResult_(true,false,"INVALID_USER",{message:"Не удалось определить пользователя."});
  if(intent.intent==="CONFIRM"||intent.intent==="CANCEL"){const found=deps.find_capture(userId,chatId,{now:now});if(!found||!found.ok)return workoutActualResult_(false,true,"NO_WORKOUT_CONFIRMATION");if(String(found.capture.user_id)!==userId||String(found.capture.chat_id)!==chatId)return workoutActualResult_(true,false,"OWNER_MISMATCH",{message:"Это подтверждение принадлежит другому пользователю или чату."});if(intent.intent==="CANCEL"){const cancelled=deps.cancel_capture(userId,chatId,{now:now});return workoutActualResult_(true,!!(cancelled&&cancelled.ok),cancelled&&cancelled.code||"CANCEL_FAILED",{message:cancelled&&cancelled.ok?"Действие отменено.":"Не удалось отменить действие."});}return executeWorkoutPendingAction_(found,userId,now,deps);}
  if(intent.intent.indexOf("HISTORY_")===0||intent.intent==="PROGRESSION_EXERCISE")return workoutHistoryIntentResult_(userId,intent,now,options||{});if(intent.intent==="INVALID_SET")return workoutActualResult_(true,false,"INVALID_SET",{message:"Не удалось однозначно разобрать подход. Ничего не записано."});const runtime=Object.assign({},options||{},{chat_id:chatId});return executeWorkoutIntent_(userId,intent,String(update.update_id||""),now,runtime);}

/* WEIGHT TRENDS + RECOVERY V1 — read-only authoritative factual projections. */
const WEIGHT_TREND_DIRECTION_THRESHOLD_KG = 0.5;

function weightTrendResult_(handled,ok,code,extra){return Object.assign({handled:handled===true,ok:ok===true,code:String(code||""),groq_calls:0,sheet_writes:0,production_writes:false},extra||{});}
function weightTrendNumber_(value){const number=Number(String(value==null?"":value).trim().replace(",","."));return isFinite(number)?number:null;}
function weightTrendRound_(value,digits){if(value==null||!isFinite(value))return null;const factor=Math.pow(10,digits==null?2:digits);return Math.round((Number(value)+Number.EPSILON)*factor)/factor;}
function weightTrendFormatNumber_(value){return value==null?"—":String(weightTrendRound_(value,2)).replace(".",",");}
function weightTrendLocalDate_(date,dependencies){return nutritionCalendarDate_(date,dependencies.time_zone(),0,dependencies.format_date);}
function weightTrendCalendarDayNumber_(date){const p=String(date||"").split("-").map(Number);return p.length===3?Math.floor(Date.UTC(p[0],p[1]-1,p[2])/86400000):null;}
function weightTrendCalendarAge_(fromDate,toDate){const from=weightTrendCalendarDayNumber_(fromDate),to=weightTrendCalendarDayNumber_(toDate);return from==null||to==null?null:to-from;}
function weightTrendShiftDate_(now,offset,deps){return nutritionCalendarDate_(now,deps.time_zone(),offset,deps.format_date);}
function weightTrendMemoryRows_(){const table=memoryReadSheetTable_(MEMORY_LAYER_CONFIG.MEMORY_SHEET),indexes={};table.headers.forEach(function(h,i){indexes[String(h||"").trim().toLowerCase()]=i;});return table.displayRows.map(function(row){const result={};Object.keys(indexes).forEach(function(key){result[key]=row[indexes[key]];});return result;});}
function weightTrendProfile_ (userId){const identity=resolveSafeContextIdentity_(userId,getSpreadsheet_());if(!identity||!identity.ok)return {};function field(name){const index=safeContextFieldIndex_(identity.headers,[name]);return index<0?"":identity.row[index];}return {start_weight:field("Вес старт"),target_weight:field("Целевой вес"),profile_current_weight:field("Текущий вес")};}
function weightTrendDependencies_(overrides){const supplied=overrides||{};return {time_zone:supplied.time_zone||function(){return Session.getScriptTimeZone();},format_date:supplied.format_date||function(date,zone){return Utilities.formatDate(date,zone,"yyyy-MM-dd");},read_memory:supplied.read_memory||weightTrendMemoryRows_,read_profile:supplied.read_profile||weightTrendProfile_};}

function loadAuthoritativeWeightHistory_(userId,options){
  const runtime=options||{},deps=weightTrendDependencies_(runtime.dependencies),expected=String(userId==null?"":userId).trim(),now=runtime.now instanceof Date?runtime.now:new Date();
  if(!expected)return {status:"DATA_INTEGRITY_ERROR",measurements:[]};
  let rows;try{rows=deps.read_memory()||[];}catch(error){return {status:"DATA_INTEGRITY_ERROR",measurements:[]};}
  const relevant=rows.filter(function(row){return String(row&&row.user_id||"").trim()===expected&&String(row&&row.category||"").trim().toLowerCase()==="body_tracking"&&String(row&&row.key||"").trim().toLowerCase()==="weight_event";});
  if(!relevant.length)return {status:"NO_DATA",measurements:[]};
  const confirmationIds={},timestamps={},measurements=[];
  for(let index=0;index<relevant.length;index+=1){const row=relevant[index],weight=weightTrendNumber_(row.value),confirmation=String(row.confirmation_id||"").trim(),time=Date.parse(String(row.updated_at||""));
    if(!String(row.user_id||"").trim()||weight==null||weight<30||weight>350||!isFinite(time)||time>now.getTime()||!confirmation||confirmationIds[confirmation]||timestamps[String(time)])return {status:"DATA_INTEGRITY_ERROR",measurements:[]};
    confirmationIds[confirmation]=true;timestamps[String(time)]=true;const updated=new Date(time);measurements.push({weight_kg:weight,updated_at:updated.toISOString(),local_date:weightTrendLocalDate_(updated,deps),_time:time});}
  measurements.sort(function(a,b){return a._time-b._time;});return {status:"NORMAL",measurements:measurements.map(function(m){return {weight_kg:m.weight_kg,updated_at:m.updated_at,local_date:m.local_date};})};
}

function buildDailyWeightProjection_(measurements){const byDate={};(measurements||[]).forEach(function(item){const current=byDate[item.local_date];if(!current||Date.parse(item.updated_at)>Date.parse(current.updated_at))byDate[item.local_date]=item;});return Object.keys(byDate).sort().map(function(date){return {local_date:date,weight_kg:byDate[date].weight_kg,updated_at:byDate[date].updated_at};});}
function weightTrendWindow_(now,days,deps){return {start:weightTrendShiftDate_(now,-(days-1),deps),end:weightTrendShiftDate_(now,0,deps)};}
function weightTrendWeekWindows_(now,deps){const today=weightTrendShiftDate_(now,0,deps),parts=today.split("-").map(Number),weekday=new Date(Date.UTC(parts[0],parts[1]-1,parts[2])).getUTCDay(),sinceMonday=(weekday+6)%7;return {current:{start:weightTrendShiftDate_(now,-sinceMonday,deps),end:today},previous:{start:weightTrendShiftDate_(now,-sinceMonday-7,deps),end:weightTrendShiftDate_(now,-7,deps)}};}
function weightTrendSelect_(daily,window){return daily.filter(function(item){return item.local_date>=window.start&&item.local_date<=window.end;});}
function weightTrendAverage_(items){return items.length?weightTrendRound_(items.reduce(function(sum,item){return sum+item.weight_kg;},0)/items.length,3):null;}
function weightTrendDelta_(items){return items.length>=2?weightTrendRound_(items[items.length-1].weight_kg-items[0].weight_kg,3):null;}
function weightTrendDirection_(delta){if(delta==null)return null;if(delta < -WEIGHT_TREND_DIRECTION_THRESHOLD_KG)return "DOWN";if(delta > WEIGHT_TREND_DIRECTION_THRESHOLD_KG)return "UP";return "STABLE";}
function weightGoalFacts_(current,profile){const start=weightTrendNumber_(profile&&profile.start_weight),target=weightTrendNumber_(profile&&profile.target_weight),validStart=start!=null&&start>=30&&start<=350,validTarget=target!=null&&target>=30&&target<=350;let type="UNKNOWN",distance=null,progress=null,reached=null;if(validStart&&validTarget)type=start>target?"LOSS":start<target?"GAIN":"MAINTAIN";if(validTarget&&current!=null)distance=weightTrendRound_(Math.abs(current-target),3);if(type==="LOSS"&&current!=null)reached=current<=target;else if(type==="GAIN"&&current!=null)reached=current>=target;else if(type==="MAINTAIN"&&current!=null)reached=current===target;if(validStart&&validTarget&&current!=null&&start!==target){const raw=(type==="LOSS"?(start-current)/(start-target):(current-start)/(target-start))*100;progress=weightTrendRound_(Math.max(0,Math.min(100,raw)),1);}return {start_weight:validStart?start:null,target_weight:validTarget?target:null,goal_type:type,kg_to_goal:distance,progress_percent:progress,goal_reached:reached};}

function buildWeightTrendFacts_(userId,options){const runtime=options||{},deps=weightTrendDependencies_(runtime.dependencies),now=runtime.now instanceof Date?runtime.now:new Date(),history=loadAuthoritativeWeightHistory_(userId,{now:now,dependencies:deps});let profile={};try{profile=deps.read_profile(String(userId))||{};}catch(ignored){}const profileFallback=weightTrendNumber_(profile.profile_current_weight),empty={data_status:history.status,freshness:history.status==="NO_DATA"?"UNKNOWN":history.status==="NORMAL"?"FRESH":"UNKNOWN",trend_status:"INSUFFICIENT_DATA",current_weight:null,current_date:null,current_time:null,measurement_age_days:null,measurements_count:0,distinct_measurement_days:0,avg_7d:null,avg_14d:null,avg_30d:null,delta_7d:null,delta_14d:null,delta_30d:null,current_week_avg:null,previous_matched_week_avg:null,week_delta:null,trend:null,goal:weightGoalFacts_(null,profile),profile_weight_fallback:profileFallback!=null&&profileFallback>=30&&profileFallback<=350?profileFallback:null};if(history.status!=="NORMAL")return empty;
  const daily=buildDailyWeightProjection_(history.measurements),latest=history.measurements[history.measurements.length-1],today=weightTrendShiftDate_(now,0,deps),age=weightTrendCalendarAge_(latest.local_date,today),facts=empty;facts.freshness=age<=7?"FRESH":"STALE";facts.current_weight=latest.weight_kg;facts.current_date=latest.local_date;facts.current_time=latest.updated_at;facts.measurement_age_days=age;facts.measurements_count=history.measurements.length;facts.distinct_measurement_days=daily.length;[7,14,30].forEach(function(days){const items=weightTrendSelect_(daily,weightTrendWindow_(now,days,deps));facts["avg_"+days+"d"]=weightTrendAverage_(items);facts["delta_"+days+"d"]=weightTrendDelta_(items);});const weeks=weightTrendWeekWindows_(now,deps),current=weightTrendSelect_(daily,weeks.current),previous=weightTrendSelect_(daily,weeks.previous);facts.current_week_avg=weightTrendAverage_(current);facts.previous_matched_week_avg=weightTrendAverage_(previous);facts.week_delta=facts.current_week_avg==null||facts.previous_matched_week_avg==null?null:weightTrendRound_(facts.current_week_avg-facts.previous_matched_week_avg,3);if(current.length>=2&&previous.length>=2){facts.trend_status="AVAILABLE";facts.trend=weightTrendDirection_(facts.week_delta);}facts.goal=weightGoalFacts_(facts.current_weight,profile);return facts;}

function detectWeightTrendIntent_(text){const n=normalizeNutritionTargetQueryText_(text);if(!n||/\d\s*(?:кг|килограмм)/.test(n)||/^(?:да|нет|отмена|отменить)$/.test(n))return null;if(/^(?:какой у меня (?:сейчас )?вес|мой текущий вес)$/.test(n))return {intent:"WEIGHT_CURRENT"};if(/^сколько.*остал.*цел.*вес/.test(n))return {intent:"WEIGHT_GOAL"};if(/(?:сравни (?:мой )?вес.*(?:прошл|предыдущ).*недел|сравни .* вес.*(?:прошл|предыдущ).*недел|вес по неделям|вес.*по сравнению с (?:прошл|предыдущ).*недел)/.test(n))return {intent:"WEIGHT_WEEK_COMPARE"};if(/вес/.test(n)&&/(?:за неделю|за 7 дн|за 14 дн|за месяц|за 30 дн)/.test(n)){const days=/(?:14)/.test(n)?14:/(?:30|месяц)/.test(n)?30:7;return {intent:"WEIGHT_PERIOD",days:days};}if(/^(?:как меняется вес|что с весом|какой тренд веса)$/.test(n))return {intent:"WEIGHT_TREND"};return null;}
function formatWeightTrendFacts_(facts,intent){if(facts.data_status==="DATA_INTEGRITY_ERROR")return "Не удалось надёжно прочитать историю веса из-за ошибки данных.";if(facts.data_status==="NO_DATA"){if(intent.intent==="WEIGHT_CURRENT"&&facts.profile_weight_fallback!=null)return "Последнего подтверждённого измерения веса нет. В профиле указано "+weightTrendFormatNumber_(facts.profile_weight_fallback)+" кг.";return "Подтверждённых измерений веса пока нет.";}if(intent.intent==="WEIGHT_CURRENT")return (facts.freshness==="STALE"?"Последний подтверждённый вес: ":"Текущий вес: ")+weightTrendFormatNumber_(facts.current_weight)+" кг. Измерение "+(facts.measurement_age_days===0?"сегодня":facts.measurement_age_days+" дн. назад")+".";if(intent.intent==="WEIGHT_GOAL")return facts.goal.target_weight==null?"Целевой вес в профиле не указан.":"До целевого веса: "+weightTrendFormatNumber_(facts.goal.kg_to_goal)+" кг."+(facts.goal.progress_percent==null?"":" Прогресс: "+weightTrendFormatNumber_(facts.goal.progress_percent)+"%.");if(intent.intent==="WEIGHT_PERIOD"){const avg=facts["avg_"+intent.days+"d"],delta=facts["delta_"+intent.days+"d"];return avg==null?"За этот период нет измерений.":"За последние "+intent.days+" дней средний вес — "+weightTrendFormatNumber_(avg)+" кг."+(delta==null?" Для изменения от первого до последнего нужно больше дней.":" От первого до последнего: "+(delta>0?"+":"")+weightTrendFormatNumber_(delta)+" кг.");}if(facts.trend_status!=="AVAILABLE")return "Для надёжного сравнения недель пока недостаточно измерений.";return "Средний вес этой недели: "+weightTrendFormatNumber_(facts.current_week_avg)+" кг.\nЗа такой же период прошлой недели: "+weightTrendFormatNumber_(facts.previous_matched_week_avg)+" кг.\nРазница: "+(facts.week_delta>0?"+":"")+weightTrendFormatNumber_(facts.week_delta)+" кг.";}
function routeWeightTrends_(update,options){const message=update&&(update.message||update.edited_message),intent=message&&typeof message.text==="string"?detectWeightTrendIntent_(message.text):null;if(!intent)return weightTrendResult_(false,true,"NOT_WEIGHT_TREND");const userId=String(message.from&&message.from.id||"").trim();if(!userId)return weightTrendResult_(true,false,"INVALID_USER",{message:"Не удалось определить пользователя."});const facts=buildWeightTrendFacts_(userId,options||{});return weightTrendResult_(true,facts.data_status!=="DATA_INTEGRITY_ERROR",facts.data_status,{facts:facts,message:formatWeightTrendFacts_(facts,intent)});}

function recoveryResult_(handled,ok,code,extra){return Object.assign({handled:handled===true,ok:ok===true,code:String(code||""),groq_calls:0,sheet_writes:0,production_writes:false},extra||{});}
function recoveryDependencies_(overrides){const supplied=overrides||{};return {time_zone:supplied.time_zone||function(){return Session.getScriptTimeZone();},format_date:supplied.format_date||function(date,zone){return Utilities.formatDate(date,zone,"yyyy-MM-dd");},load_history:supplied.load_history||function(userId){return loadEffectiveWorkoutHistory_(userId,{dependencies:supplied.history_dependencies});},resolve_plan:supplied.resolve_plan||function(userId,now){return resolveWorkoutPlan_(userId,{intent:"WORKOUT_TODAY"},{now:now,dependencies:supplied.plan_dependencies});}};}
function buildRecoveryFacts_(userId,options){const runtime=options||{},deps=recoveryDependencies_(runtime.dependencies),now=runtime.now instanceof Date?runtime.now:new Date(),today=weightTrendLocalDate_(now,deps);let history;try{history=deps.load_history(String(userId));}catch(error){history=null;}const facts={history_status:"NO_DATA",last_completed_workout:null,sessions_7d:0,sessions_14d:0,training_days_7d:0,training_days_14d:0,plan_status:"PLAN_NOT_CONFIGURED",planned_today:null,next_workout:null};if(!history||history.ok!==true){facts.history_status="DATA_INTEGRITY_ERROR";}else{const completed=(history.sessions||[]).filter(function(s){return s.status==="COMPLETED"&&s.completed_at instanceof Date&&isFinite(s.completed_at.getTime());}).sort(function(a,b){return b.completed_at-a.completed_at;});if(completed.length>1&&completed[0].completed_at.getTime()===completed[1].completed_at.getTime()){facts.history_status="DATA_INTEGRITY_ERROR";}else{facts.history_status=completed.length?"NORMAL":"NO_DATA";[7,14].forEach(function(days){const start=weightTrendShiftDate_(now,-(days-1),deps),dates={};const selected=completed.filter(function(s){const date=weightTrendLocalDate_(s.started_at,deps);if(date>=start&&date<=today){dates[date]=true;return true;}return false;});facts["sessions_"+days+"d"]=selected.length;facts["training_days_"+days+"d"]=Object.keys(dates).length;});if(completed.length){const last=completed[0],date=weightTrendLocalDate_(last.started_at,deps);facts.last_completed_workout={date:date,session_name:last.display_name,completed_at:last.completed_at.toISOString(),days_ago:weightTrendCalendarAge_(date,today)};}}}
  let plan;try{plan=deps.resolve_plan(String(userId),now);}catch(error){plan=null;}if(!plan||plan.ok!==true)facts.plan_status="DATA_INTEGRITY_ERROR";else{facts.plan_status=["TRAINING_DAY","REST_DAY","PLAN_NOT_CONFIGURED"].indexOf(plan.status)>=0?plan.status:"DATA_INTEGRITY_ERROR";if(plan.status==="TRAINING_DAY"&&plan.workout)facts.planned_today={session_name:plan.workout.display_name};if(plan.next_workout){facts.next_workout={date:plan.next_workout.local_date,session_name:plan.next_workout.workout.display_name,days_away:weightTrendCalendarAge_(today,plan.next_workout.local_date)};}}return facts;}
function detectRecoveryIntent_(text){const n=normalizeNutritionTargetQueryText_(text);if(!n)return null;if(/^(?:когда была последняя тренировка|когда я тренировался в последний раз)$/.test(n))return {intent:"RECOVERY_LAST"};if(/^(?:сколько тренировок за неделю|сколько я тренировался за последние 14 дней|как часто я тренируюсь)$/.test(n))return {intent:"RECOVERY_FREQUENCY",days:/14/.test(n)?14:7};if(/^(?:как я восстановился|как я восстанавливаюсь|как восстановление сегодня|что по восстановлению|что у меня по восстановлению|что сегодня по восстановлению|сколько дней я отдыхаю)$/.test(n))return {intent:"RECOVERY_CONTEXT"};return null;}
function buildRecoveryContext_(userId,options){const runtime=options||{},objective=buildRecoveryFacts_(userId,runtime),read=runtime.load_subjective||loadAuthoritativeSubjectiveRecovery_;let subjective;try{subjective=read(String(userId),{now:runtime.now instanceof Date?runtime.now:new Date(),dependencies:runtime.subjective_dependencies});}catch(error){subjective=subjectiveRecoveryEmptyProjection_("DATA_INTEGRITY_ERROR","");}return {objective:objective,subjective:subjective};}
function formatRecoveryContext_(context){const objective=context.objective||{},subjective=context.subjective||{},lines=[],labels={sleep_duration_hours:"Сон",sleep_quality:"Качество сна",fatigue:"Усталость",soreness:"Мышечная болезненность",stress:"Стресс",wellbeing:"Самочувствие",readiness:"Готовность"};const current=[];SUBJECTIVE_RECOVERY_METRICS.forEach(function(key){const metric=subjective.metrics&&subjective.metrics[key];if(metric&&metric.status==="FRESH")current.push(labels[key]+": "+dailyNutritionNumber_(metric.value,key==="sleep_duration_hours"?2:0)+(key==="sleep_duration_hours"?" ч":"/10"));});if(current.length)lines.push("Сегодня ты отметил:",current.join("\n"));else if(subjective.data_status==="DATA_INTEGRITY_ERROR")lines.push("Субъективные данные восстановления сейчас недоступны.");else lines.push("Сегодня субъективных отметок пока нет.");if(objective.history_status==="DATA_INTEGRITY_ERROR")lines.push("Историю тренировок не удалось надёжно прочитать.");else if(objective.last_completed_workout)lines.push("Последняя завершённая тренировка была "+(objective.last_completed_workout.days_ago===0?"сегодня":objective.last_completed_workout.days_ago===1?"вчера":objective.last_completed_workout.days_ago+" дн. назад")+".");else lines.push("Завершённых тренировок пока нет.");if(objective.plan_status==="TRAINING_DAY"&&objective.planned_today)lines.push("Сегодня по плану: "+objective.planned_today.session_name+".");else if(objective.plan_status==="REST_DAY")lines.push("Сегодня по плану день отдыха.");return lines.join("\n");}
function formatRecoveryFacts_(facts,intent){const historyError=facts.history_status==="DATA_INTEGRITY_ERROR",planError=facts.plan_status==="DATA_INTEGRITY_ERROR";if(intent.intent==="RECOVERY_LAST"){if(historyError)return "Не удалось надёжно прочитать историю тренировок.";return facts.last_completed_workout?"Последняя завершённая тренировка: "+facts.last_completed_workout.date+" — "+facts.last_completed_workout.session_name+" ("+facts.last_completed_workout.days_ago+" дн. назад).":"Завершённых тренировок пока нет.";}if(intent.intent==="RECOVERY_FREQUENCY"){if(historyError)return "Не удалось надёжно прочитать историю тренировок.";const days=intent.days||7;return "За последние "+days+" дней: "+facts["sessions_"+days+"d"]+" завершённых тренировок в "+facts["training_days_"+days+"d"]+" тренировочных дней.";}const lines=[];if(historyError)lines.push("Историю тренировок не удалось надёжно прочитать.");else if(facts.last_completed_workout)lines.push("Последняя тренировка была "+facts.last_completed_workout.days_ago+" дн. назад. За 7 дней: "+facts.sessions_7d+" тренировок.");else lines.push("Завершённых тренировок пока нет.");if(planError)lines.push("План тренировок содержит ошибку данных.");else if(facts.plan_status==="TRAINING_DAY")lines.push("Сегодня по плану: "+facts.planned_today.session_name+".");else if(facts.plan_status==="REST_DAY")lines.push("Сегодня по плану день отдыха.");lines.push("Физиологическое восстановление определить нельзя: сон, усталость и самочувствие не логируются authoritative.");return lines.join("\n");}
function routeRecoveryFacts_(update,options){const message=update&&(update.message||update.edited_message),intent=message&&typeof message.text==="string"?detectRecoveryIntent_(message.text):null;if(!intent)return recoveryResult_(false,true,"NOT_RECOVERY_FACT");const userId=String(message.from&&message.from.id||"").trim();if(!userId)return recoveryResult_(true,false,"INVALID_USER",{message:"Не удалось определить пользователя."});if(intent.intent==="RECOVERY_CONTEXT"){const context=buildRecoveryContext_(userId,options||{});return recoveryResult_(true,true,"RECOVERY_CONTEXT",{facts:context,message:formatRecoveryContext_(context)});}const facts=buildRecoveryFacts_(userId,options||{});return recoveryResult_(true,true,"RECOVERY_FACTS",{facts:facts,message:formatRecoveryFacts_(facts,intent)});}

/* DASH-1 — read-only orchestration over authoritative domain fact boundaries. */
function dailyDashboardResult_(handled,ok,code,extra){return Object.assign({handled:handled===true,ok:ok===true,code:String(code||""),groq_calls:0,sheet_writes:0,domain_writes:0,pending_capture_writes:0,production_writes:false},extra||{});}
function detectDailyDashboardIntent_(text){const n=normalizeNutritionTargetQueryText_(text);if(!n)return null;return /^(?:что у меня сегодня|дай сводку на сегодня|покажи сводку на сегодня|как дела на сегодня|как у меня дела|как идет прогресс|какой статус на сегодня|подведи итог за сегодня|сводка на сегодня)$/.test(n)?{intent:"DAILY_DASHBOARD"}:null;}
function dailyDashboardDependencies_(overrides){const supplied=overrides||{};return {time_zone:supplied.time_zone||function(){return Session.getScriptTimeZone();},format_date:supplied.format_date||function(date,zone){return Utilities.formatDate(date,zone,"yyyy-MM-dd");},build_weight:supplied.build_weight||buildWeightTrendFacts_,load_consumed:supplied.load_consumed||loadDailyNutritionSummary_,load_targets:supplied.load_targets||loadAuthoritativeNutritionTargets_,build_recovery:supplied.build_recovery||buildRecoveryFacts_,load_subjective:supplied.load_subjective||(supplied.build_recovery?function(){return subjectiveRecoveryEmptyProjection_("NO_DATA","");}:loadAuthoritativeSubjectiveRecovery_),log_error:supplied.log_error||function(scope){try{console.error("Daily dashboard section unavailable: "+scope);}catch(ignored){}}};}
function dailyDashboardWeightProjection_(facts,state){const source=facts||{},goal=source.goal||{};return {available:state==="OK"&&source.data_status==="NORMAL"&&source.current_weight!=null,status:state==="OK"?source.data_status:"UNAVAILABLE",data_status:state==="OK"?String(source.data_status||"DATA_INTEGRITY_ERROR"):"UNAVAILABLE",freshness:state==="OK"?String(source.freshness||"UNKNOWN"):"UNKNOWN",trend_status:state==="OK"?String(source.trend_status||"INSUFFICIENT_DATA"):"INSUFFICIENT_DATA",current_weight:source.current_weight==null?null:Number(source.current_weight),current_date:source.current_date||null,measurement_age_days:source.measurement_age_days==null?null:Number(source.measurement_age_days),trend:source.trend||null,week_delta:source.week_delta==null?null:Number(source.week_delta),target_weight:goal.target_weight==null?null:Number(goal.target_weight),kg_to_goal:goal.kg_to_goal==null?null:Number(goal.kg_to_goal),progress_percent:goal.progress_percent==null?null:Number(goal.progress_percent)};}
function dailyDashboardNutritionProjection_(consumed,targets,consumedState,targetState){const c=consumed||{},t=targets&&targets.targets||{calories:null,protein:null,fat:null,carbs:null},hasRecords=consumedState==="OK"&&Number(c.meals_count)>0,requested=["calories","protein","fat","carbs"];let remaining={calories:null,protein:null,fat:null,carbs:null};if(hasRecords)remaining=calculateRemainingNutritionTargets_(t,c.consumed,requested).remaining;return {available:consumedState==="OK",status:consumedState!=="OK"?"UNAVAILABLE":hasRecords?"AVAILABLE":"NO_RECORDS",has_records:hasRecords,consumed:hasRecords?Object.assign({},c.consumed):null,target_status:targetState!=="OK"?"UNAVAILABLE":String(targets.status||"NOT_CONFIGURED"),targets:targetState==="OK"?Object.assign({},t):{calories:null,protein:null,fat:null,carbs:null},remaining:remaining};}
function dailyDashboardTrainingProjection_(facts,state){const status=state==="OK"?String(facts&&facts.plan_status||"DATA_INTEGRITY_ERROR"):"UNAVAILABLE";return {available:state==="OK"&&status!=="DATA_INTEGRITY_ERROR",status:status,today_session_name:status==="TRAINING_DAY"&&facts.planned_today?facts.planned_today.session_name:null,next_workout:facts&&facts.next_workout?Object.assign({},facts.next_workout):null};}
function dailyDashboardRecoveryProjection_(facts,state,subjective,subjectiveState){const status=state==="OK"?String(facts&&facts.history_status||"DATA_INTEGRITY_ERROR"):"UNAVAILABLE",metrics={};SUBJECTIVE_RECOVERY_METRICS.forEach(function(key){const metric=subjective&&subjective.metrics&&subjective.metrics[key];metrics[key]=subjectiveState==="OK"&&metric&&metric.status==="FRESH"?Number(metric.value):null;});return {available:(state==="OK"&&status!=="DATA_INTEGRITY_ERROR")||subjectiveState==="OK",history_status:status,last_completed_workout:facts&&facts.last_completed_workout?Object.assign({},facts.last_completed_workout):null,sessions_7d:status==="NORMAL"?Number(facts.sessions_7d):0,sessions_14d:status==="NORMAL"?Number(facts.sessions_14d):0,training_days_7d:status==="NORMAL"?Number(facts.training_days_7d):0,training_days_14d:status==="NORMAL"?Number(facts.training_days_14d):0,subjective_status:subjectiveState!=="OK"?"UNAVAILABLE":String(subjective&&subjective.data_status||"NO_DATA"),today:metrics};}
function buildDailyDashboardFacts_(userId,options){const runtime=options||{},deps=dailyDashboardDependencies_(runtime.dependencies),now=runtime.now instanceof Date?runtime.now:new Date(),localDate=nutritionCalendarDate_(now,deps.time_zone(),0,deps.format_date);let weight=null,consumed=null,targets=null,recovery=null,subjective=null,ws="OK",cs="OK",ts="OK",rs="OK",ss="OK";try{weight=deps.build_weight(String(userId),runtime.weight_options||{});}catch(error){ws="UNAVAILABLE";deps.log_error("weight");}try{consumed=deps.load_consumed(String(userId),runtime.nutrition_options||{});if(!consumed||consumed.ok!==true)cs="ERROR";}catch(error){cs="UNAVAILABLE";deps.log_error("nutrition_consumed");}try{targets=deps.load_targets(String(userId),runtime.target_options||{});if(!targets||targets.ok!==true)ts="ERROR";}catch(error){ts="UNAVAILABLE";deps.log_error("nutrition_targets");}try{recovery=deps.build_recovery(String(userId),runtime.recovery_options||{});}catch(error){rs="UNAVAILABLE";deps.log_error("recovery");}try{subjective=deps.load_subjective(String(userId),{now:now,dependencies:runtime.subjective_dependencies});if(!subjective||subjective.data_status==="DATA_INTEGRITY_ERROR")ss="ERROR";}catch(error){ss="UNAVAILABLE";deps.log_error("subjective_recovery");}const result={local_date:localDate,weight:dailyDashboardWeightProjection_(weight,ws),nutrition:dailyDashboardNutritionProjection_(consumed,targets,cs,ts),training:dailyDashboardTrainingProjection_(recovery,rs),recovery:dailyDashboardRecoveryProjection_(recovery,rs,subjective,ss)};result.has_any_useful_section=result.weight.available||result.nutrition.available||result.training.available||result.recovery.available;result.has_domain_errors=[result.weight.data_status,result.nutrition.status,result.nutrition.target_status,result.training.status,result.recovery.history_status,result.recovery.subjective_status].some(function(status){return status==="DATA_INTEGRITY_ERROR"||status==="UNAVAILABLE";});return result;}
function dailyDashboardNumber_(value,digits){return dailyNutritionNumber_(value,digits==null?1:digits);}
function dailyDashboardDateTitle_(date){const p=String(date||"").split("-").map(Number),months=["января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"];return p.length===3&&months[p[1]-1]?p[2]+" "+months[p[1]-1]:String(date||"");}
function formatDailyDashboard_(facts){const lines=["Сегодня, "+dailyDashboardDateTitle_(facts.local_date),""];const training=facts.training;if(training.status==="TRAINING_DAY")lines.push("Тренировка: "+training.today_session_name+".");else if(training.status==="REST_DAY")lines.push("Тренировка: сегодня по плану день отдыха.");else if(training.status==="PLAN_NOT_CONFIGURED")lines.push("Тренировка: план пока не настроен.");else lines.push("Тренировка: сейчас не удалось получить корректные данные.");if(training.next_workout&&training.status!=="TRAINING_DAY")lines.push("Следующая: "+training.next_workout.session_name+" — через "+training.next_workout.days_away+" дн.");lines.push("");const nutrition=facts.nutrition;if(nutrition.status==="UNAVAILABLE")lines.push("Питание: сейчас не удалось получить корректные данные.");else if(nutrition.status==="NO_RECORDS")lines.push("Питание: сегодня записей пока нет.");else{const c=nutrition.consumed,t=nutrition.targets,r=nutrition.remaining,cal=t.calories==null?dailyDashboardNumber_(c.calories,1)+" ккал":dailyDashboardNumber_(c.calories,1)+" из "+dailyDashboardNumber_(t.calories,1)+" ккал";lines.push("Питание: "+cal+".");if(r.calories!=null)lines.push((r.calories>=0?"Осталось: ":"Превышение: ")+dailyDashboardNumber_(Math.abs(r.calories),1)+" ккал.");const labels={protein:"Б",fat:"Ж",carbs:"У"},macros=["protein","fat","carbs"].map(function(key){return labels[key]+" "+dailyDashboardNumber_(c[key],1)+(t[key]==null?"":"/"+dailyDashboardNumber_(t[key],1));});lines.push(macros.join(" · "));if(nutrition.target_status==="UNAVAILABLE")lines.push("Цели по питанию: сейчас не удалось получить корректные данные.");}lines.push("");const weight=facts.weight;if(weight.data_status==="UNAVAILABLE"||weight.data_status==="DATA_INTEGRITY_ERROR")lines.push("Вес: сейчас не удалось получить корректные данные.");else if(weight.data_status==="NO_DATA")lines.push("Вес: подтверждённых измерений пока нет.");else{let line="Вес: "+dailyDashboardNumber_(weight.current_weight,2)+" кг";if(weight.measurement_age_days===0)line+=" — измерение сегодня";else line+=" — измерение "+weight.measurement_age_days+" дн. назад";if(weight.freshness==="FRESH"&&weight.trend_status==="AVAILABLE"&&weight.week_delta!=null)line+=". Тренд недели: "+(weight.week_delta>0?"+":"")+dailyDashboardNumber_(weight.week_delta,2)+" кг";lines.push(line+".");}lines.push("");const recovery=facts.recovery;if(recovery.history_status==="UNAVAILABLE"||recovery.history_status==="DATA_INTEGRITY_ERROR")lines.push("История тренировок: сейчас не удалось получить корректные данные.");else if(recovery.history_status==="NO_DATA")lines.push("История тренировок: завершённых тренировок пока нет.");else{const last=recovery.last_completed_workout;lines.push("Последняя тренировка: "+(last.days_ago===0?"сегодня":last.days_ago===1?"вчера":last.days_ago+" дн. назад")+".");lines.push("За 7 дней: "+recovery.sessions_7d+" тренировок.");}const subjective=[];const sl={sleep_duration_hours:"Сон",fatigue:"усталость",soreness:"забитость",stress:"стресс",wellbeing:"самочувствие",readiness:"готовность"};Object.keys(sl).forEach(function(key){const value=recovery.today&&recovery.today[key];if(value!==null&&value!==undefined)subjective.push(sl[key]+" "+dailyDashboardNumber_(value,key==="sleep_duration_hours"?2:0)+(key==="sleep_duration_hours"?" ч":"/10"));});if(subjective.length)lines.push("Восстановление сегодня: "+subjective.join(", ")+".");return lines.join("\n").replace(/\n{3,}/g,"\n\n");}
function routeDailyDashboard_(update,options){const message=update&&(update.message||update.edited_message),intent=message&&typeof message.text==="string"?detectDailyDashboardIntent_(message.text):null;if(!intent)return dailyDashboardResult_(false,true,"NOT_DAILY_DASHBOARD");const userId=String(message.from&&message.from.id||"").trim();if(!userId)return dailyDashboardResult_(true,false,"INVALID_USER",{message:"Не удалось сформировать сводку на сегодня."});const runtime=options||{},facts=buildDailyDashboardFacts_(userId,runtime),primary=formatDailyDashboard_(facts);let messageText=primary,hint=null;try{const evaluate=runtime.proactive_evaluate||evaluateContextualCoachingHint_;hint=evaluate(facts,runtime.now instanceof Date?runtime.now:new Date(),PROACTIVE_COACHING_CONTEXTS.DASHBOARD,runtime.proactive_options);messageText=appendContextualCoachingHint_(primary,hint);}catch(ignored){}return dailyDashboardResult_(true,true,"DAILY_DASHBOARD",{facts:facts,message:messageText,coaching_hint:hint&&hint.message?hint:null});}
...
const PROACTIVE_COACHING_PRIORITIES = Object.freeze({
  CALORIES_EXCEEDED:1,
  SUBJECTIVE_HIGH_FATIGUE:2,
  SUBJECTIVE_LOW_READINESS:2,
  SUBJECTIVE_SHORT_SLEEP:3,
...
function detectProactiveCoachingSignals_(facts,now,options){
  ...
  try{
    const recovery=source.recovery||{},today=recovery.today||{};
    if(recovery.subjective_status==="NORMAL"){
      if(proactiveCoachingFinite_(today.fatigue)&&Number(today.fatigue)>=8)result.push(proactiveCoachingSignal_("SUBJECTIVE_HIGH_FATIGUE",{fatigue:Number(today.fatigue)}));
      if(proactiveCoachingFinite_(today.readiness)&&Number(today.readiness)<=3)result.push(proactiveCoachingSignal_("SUBJECTIVE_LOW_READINESS",{readiness:Number(today.readiness)}));
      if(proactiveCoachingFinite_(today.sleep_duration_hours)&&Number(today.sleep_duration_hours)<6)result.push(proactiveCoachingSignal_("SUBJECTIVE_SHORT_SLEEP",{sleep_hours:Number(today.sleep_duration_hours)}));
    }
  }catch(ignoredRecovery){}
  return result;
}
...
/* Minimal proactive delivery: one user-scoped message per local day, outside quiet hours. */
const PROACTIVE_DELIVERY_ENABLED_PROPERTY="PROACTIVE_DELIVERY_ENABLED";
const PROACTIVE_DELIVERY_STATE_PREFIX="PROACTIVE_DELIVERY_STATE_";
function proactiveDeliveryResult_(ok,code,extra){return Object.assign({ok:ok===true,code:String(code||""),sent:0,skipped:0,groq_calls:0,production_writes:false},extra||{});}
function proactiveDeliveryDependencies_(overrides){const supplied=overrides||{},properties=supplied.properties||(typeof PropertiesService!=="undefined"?PropertiesService.getScriptProperties():null);return {now:supplied.now||function(){return new Date();},properties:properties,list_users:supplied.list_users||function(){const sheet=getSpreadsheet_().getSheetByName("User_Profile");if(!sheet)return [];const values=sheet.getDataRange().getValues(),headers=values[0]||[],index=headers.indexOf("Telegram_ID");if(index<0)return [];const seen={};return values.slice(1).map(function(row){return String(row[index]||"").trim();}).filter(function(id){if(!/^\d+$/.test(id)||seen[id])return false;seen[id]=true;return true;});},build_facts:supplied.build_facts||buildProactiveCoachingFacts_,evaluate:supplied.evaluate||evaluateProactiveCoaching_,send:supplied.send||sendTelegramMessage_,time_zone:supplied.time_zone||function(){return Session.getScriptTimeZone();},format_date:supplied.format_date||function(date,zone,pattern){return Utilities.formatDate(date,zone,pattern);}};}
function proactiveDeliveryStateKey_(userId){return PROACTIVE_DELIVERY_STATE_PREFIX+String(userId).replace(/[^0-9]/g,"");}
function evaluateProactiveDelivery_(userId,now,options){const runtime=options||{},deps=proactiveDeliveryDependencies_(runtime.dependencies),zone=deps.time_zone(),hour=Number(deps.format_date(now,zone,"H")),date=deps.format_date(now,zone,"yyyy-MM-dd");if(!isFinite(hour)||hour<8||hour>=22)return proactiveDeliveryResult_(true,"QUIET_HOURS",{skipped:1});let state={};try{state=JSON.parse(deps.properties.getProperty(proactiveDeliveryStateKey_(userId))||"{}");}catch(ignored){state={};}if(state.local_date===date&&state.sent===true)return proactiveDeliveryResult_(true,"DAILY_LIMIT",{skipped:1});if(state.sent_at&&now.getTime()-Date.parse(state.sent_at)<20*60*60*1000)return proactiveDeliveryResult_(true,"COOLDOWN",{skipped:1});let facts,evaluation;try{facts=deps.build_facts(String(userId),now,runtime.fact_options);evaluation=deps.evaluate(facts,now,runtime.signal_options);}catch(error){return proactiveDeliveryResult_(false,"FACTS_UNAVAILABLE",{skipped:1});}if(!evaluation||!evaluation.selected_signal||!evaluation.message)return proactiveDeliveryResult_(true,"NO_SIGNAL",{skipped:1});return proactiveDeliveryResult_(true,"READY",{message:evaluation.message,signal_key:evaluation.selected_signal.key,state_key:proactiveDeliveryStateKey_(userId),state:{local_date:date,sent_at:now.toISOString(),sent:true,signal_key:evaluation.selected_signal.key}});}
function runProactiveCoachingDelivery_(options){const runtime=options||{},deps=proactiveDeliveryDependencies_(runtime.dependencies),enabled=deps.properties&&deps.properties.getProperty(PROACTIVE_DELIVERY_ENABLED_PROPERTY)==="true";if(!enabled)return proactiveDeliveryResult_(true,"DISABLED");const now=runtime.now instanceof Date?runtime.now:deps.now(),users=deps.list_users(),result=proactiveDeliveryResult_(true,"COMPLETED");for(let i=0;i<users.length;i++){const decision=evaluateProactiveDelivery_(users[i],now,{dependencies:deps,fact_options:runtime.fact_options,signal_options:runtime.signal_options});if(decision.code!=="READY"){result.skipped++;continue;}try{deps.send(users[i],decision.message);deps.properties.setProperty(decision.state_key,JSON.stringify(decision.state));result.sent++;}catch(error){result.ok=false;result.code="PARTIAL_FAILURE";result.skipped++;}}return result;}
function runProactiveCoachingDelivery(){return runProactiveCoachingDelivery_({});}
function ensureProactiveCoachingTrigger(){const handler="runProactiveCoachingDelivery",existing=ScriptApp.getProjectTriggers().filter(function(trigger){return trigger.getHandlerFunction()===handler;});if(existing.length)return {ok:true,created:false,count:existing.length};ScriptApp.newTrigger(handler).timeBased().everyHours(2).create();return {ok:true,created:true,count:1};}

/* PC-2 — contextual presentation of one PC-1 hint at approved response boundaries. */
const PROACTIVE_COACHING_CONTEXTS=Object.freeze({DASHBOARD:"DASHBOARD",NUTRITION_LOG_SUCCESS:"NUTRITION_LOG_SUCCESS"});
const PROACTIVE_COACHING_NUTRITION_SUCCESS_SIGNALS=Object.freeze({CALORIES_EXCEEDED:true,CALORIES_NEAR_LIMIT:true,PROTEIN_FAR_BELOW_TARGET_LATE_DAY:true});
function filterProactiveCoachingSignalsForContext_(signals,context){if(context===PROACTIVE_COACHING_CONTEXTS.DASHBOARD)return (signals||[]).slice();if(context===PROACTIVE_COACHING_CONTEXTS.NUTRITION_LOG_SUCCESS)return (signals||[]).filter(function(signal){return signal&&PROACTIVE_COACHING_NUTRITION_SUCCESS_SIGNALS[signal.key]===true;});return [];}
function evaluateContextualCoachingHint_(facts,now,context,options){const detected=detectProactiveCoachingSignals_(facts,now,options),contextual=filterProactiveCoachingSignalsForContext_(detected,context),eligible=applyProactiveCoachingPolicy_(contextual),selected=selectProactiveCoachingSignal_(eligible);return {context:context,signals:detected,contextual_signals:contextual,eligible_signals:eligible,selected_signal:selected,message:formatProactiveCoachingHint_(selected),groq_calls:0,telegram_calls:0,sheet_writes:0,domain_writes:0,pending_capture_writes:0,property_writes:0,production_writes:false};}
function appendContextualCoachingHint_(primary,evaluation){const message=evaluation&&String(evaluation.message||"").trim();return message?String(primary)+"\n\nПодсказка: "+message:String(primary);}
function appendNutritionSuccessCoachingHint_(primary,userId,now,dependencies){const deps=dependencies||{},build=deps.build_dashboard||buildDailyDashboardFacts_,evaluate=deps.evaluate_contextual||evaluateContextualCoachingHint_,facts=build(String(userId),{now:now instanceof Date?now:new Date(),dependencies:deps.dashboard_dependencies}),hint=evaluate(facts,now instanceof Date?now:new Date(),PROACTIVE_COACHING_CONTEXTS.NUTRITION_LOG_SUCCESS,deps.proactive_options);return {message:appendContextualCoachingHint_(primary,hint),hint:hint&&hint.message?hint:null};}

/* SR-1 — internal authoritative subjective-recovery persistence and reader. */
const SUBJECTIVE_RECOVERY_SHEET="Recovery_Checkin";
const SUBJECTIVE_RECOVERY_SCHEMA_VERSION="sr1-subjective-recovery-v1";
const SUBJECTIVE_RECOVERY_SOURCE="SUBJECTIVE_RECOVERY";
const SUBJECTIVE_RECOVERY_SCHEMA=Object.freeze(["SCHEMA_VERSION","EVENT_ID","IDEMPOTENCY_KEY","USER_ID","REPORTED_AT","LOCAL_DATE","SLEEP_DURATION_HOURS","SLEEP_QUALITY","FATIGUE","SORENESS","STRESS","WELLBEING","READINESS","SOURCE","CREATED_AT"]);
const SUBJECTIVE_RECOVERY_METRICS=Object.freeze(["sleep_duration_hours","sleep_quality","fatigue","soreness","stress","wellbeing","readiness"]);
function subjectiveRecoveryResult_(ok,code,extra){return Object.assign({ok:ok===true,code:String(code||""),rows_written:0,idempotent_replay:false,groq_calls:0,telegram_calls:0,ai_memory_writes:0,coach_state_writes:0,production_writes:false},extra||{});}
function validateSubjectiveRecoverySchema_(headers){return Array.isArray(headers)&&headers.length===SUBJECTIVE_RECOVERY_SCHEMA.length&&headers.every(function(value,index){return String(value)===SUBJECTIVE_RECOVERY_SCHEMA[index];});}
function subjectiveRecoveryBlank_(value){return value===null||value===undefined||value==="";}
function subjectiveRecoveryTimestamp_(value){const date=value instanceof Date?new Date(value.getTime()):new Date(value);return isFinite(date.getTime())?date:null;}
function subjectiveRecoveryCanonical_(event){const metrics={};SUBJECTIVE_RECOVERY_METRICS.forEach(function(key){if(!subjectiveRecoveryBlank_(event[key]))metrics[key]=Number(event[key]);});return JSON.stringify({schema_version:event.schema_version,user_id:event.user_id,reported_at:event.reported_at,local_date:event.local_date,metrics:metrics,source:event.source});}
function subjectiveRecoveryRecordValues_(record){return SUBJECTIVE_RECOVERY_SCHEMA.map(function(header){return record[header.toLowerCase()];});}
function subjectiveRecoveryValuesRecord_(values){const record={};SUBJECTIVE_RECOVERY_SCHEMA.forEach(function(header,index){record[header.toLowerCase()]=values[index];});return record;}
function subjectiveRecoveryCanonicalLocalDate_(value,deps){if(typeof value==="string"){const text=value.trim(),match=text.match(/^(\d{4})-(\d{2})-(\d{2})$/);if(!match)return null;const year=Number(match[1]),month=Number(match[2]),day=Number(match[3]),date=new Date(Date.UTC(year,month-1,day));return date.getUTCFullYear()===year&&date.getUTCMonth()===month-1&&date.getUTCDate()===day?text:null;}if(typeof value==="number"){if(!isFinite(value)||Math.floor(value)!==value||value<1||value>2958465)return null;const date=new Date(Date.UTC(1899,11,30)+value*86400000);return isFinite(date.getTime())?date.toISOString().slice(0,10):null;}if(value instanceof Date&&isFinite(value.getTime())&&deps&&typeof deps.time_zone==="function"&&typeof deps.format_date==="function")return subjectiveRecoveryCanonicalLocalDate_(nutritionCalendarDate_(value,deps.time_zone(),0,deps.format_date));return null;}
function subjectiveRecoveryRecordsEqual_(actual,expected,deps){return SUBJECTIVE_RECOVERY_SCHEMA.every(function(header){const key=header.toLowerCase();if(header==="LOCAL_DATE"){const a=subjectiveRecoveryCanonicalLocalDate_(actual&&actual[key],deps),e=subjectiveRecoveryCanonicalLocalDate_(expected&&expected[key],deps);return a!==null&&e!==null&&a===e;}return String(actual&&actual[key])===String(expected&&expected[key]);});}
function validateSubjectiveRecoveryEvent_(event,now,deps){if(!event||String(event.schema_version)!==SUBJECTIVE_RECOVERY_SCHEMA_VERSION||!String(event.event_id||"").trim()||!String(event.idempotency_key||"").trim()||!String(event.user_id||"").trim()||String(event.source)!==SUBJECTIVE_RECOVERY_SOURCE)return {ok:false};const reported=subjectiveRecoveryTimestamp_(event.reported_at),created=subjectiveRecoveryTimestamp_(event.created_at),localDate=subjectiveRecoveryCanonicalLocalDate_(event.local_date,deps);if(!reported||!created||!localDate||reported.getTime()>now.getTime()||created.getTime()>now.getTime()||localDate!==nutritionCalendarDate_(reported,deps.time_zone(),0,deps.format_date))return {ok:false};let count=0;for(let i=0;i<SUBJECTIVE_RECOVERY_METRICS.length;i++){const key=SUBJECTIVE_RECOVERY_METRICS[i],value=event[key];if(subjectiveRecoveryBlank_(value))continue;count++;const number=Number(value);if(!isFinite(number))return {ok:false};if(key==="sleep_duration_hours"){if(number<0||number>18)return {ok:false};}else if(number<0||number>10||Math.floor(number)!==number)return {ok:false};event[key]=number;}if(!count)return {ok:false};event.local_date=localDate;event.reported_at=reported.toISOString();event.created_at=created.toISOString();return {ok:true,event:event};}
function subjectiveRecoveryDependencies_(overrides){const supplied=overrides||{};return {time_zone:supplied.time_zone||function(){return Session.getScriptTimeZone();},format_date:supplied.format_date||function(date,zone){return Utilities.formatDate(date,zone,"yyyy-MM-dd");},read_table:supplied.read_table||function(){const sheet=getSpreadsheet_().getSheetByName(SUBJECTIVE_RECOVERY_SHEET);if(!sheet)throw new Error("RECOVERY_CHECKIN_MISSING");const last=sheet.getLastRow(),values=sheet.getRange(1,1,Math.max(1,last),SUBJECTIVE_RECOVERY_SCHEMA.length).getValues();return {headers:values[0]||[],rows:values.slice(1)};},append_row:supplied.append_row||function(values){const sheet=getSpreadsheet_().getSheetByName(SUBJECTIVE_RECOVERY_SHEET);if(!sheet)throw new Error("RECOVERY_CHECKIN_MISSING");const row=sheet.getLastRow()+1;sheet.getRange(row,1,1,SUBJECTIVE_RECOVERY_SCHEMA.length).setValues([values]);SpreadsheetApp.flush();return row;},read_row:supplied.read_row||function(row){const sheet=getSpreadsheet_().getSheetByName(SUBJECTIVE_RECOVERY_SHEET);return sheet?sheet.getRange(row,1,1,SUBJECTIVE_RECOVERY_SCHEMA.length).getValues()[0]:null;},lock:supplied.lock||function(){return LockService.getScriptLock();}};}
function subjectiveRecoveryReadEvents_(userId,table,now,deps){if(!table||!validateSubjectiveRecoverySchema_(table.headers))return {ok:false};const events=[];for(let i=0;i<(table.rows||[]).length;i++){const raw=Array.isArray(table.rows[i])?subjectiveRecoveryValuesRecord_(table.rows[i]):Object.assign({},table.rows[i]);if(String(raw.user_id)!==String(userId))continue;const valid=validateSubjectiveRecoveryEvent_(raw,now,deps);if(!valid.ok)return {ok:false};events.push(valid.event);}const ids={},keys={};for(let i=0;i<events.length;i++){const event=events[i],id=String(event.event_id),key=String(event.idempotency_key);if(ids[id]||keys[key])return {ok:false};ids[id]=event;keys[key]=event;}return {ok:true,events:events};}
function subjectiveRecoveryEmptyProjection_(status,localDate){const metrics={};SUBJECTIVE_RECOVERY_METRICS.forEach(function(key){metrics[key]={status:"MISSING",value:null};});return {data_status:status,local_date:localDate,has_current_data:false,metrics:metrics};}
function loadAuthoritativeSubjectiveRecovery_(userId,options){const runtime=options||{},deps=subjectiveRecoveryDependencies_(runtime.dependencies),now=runtime.now instanceof Date?runtime.now:new Date(),today=nutritionCalendarDate_(now,deps.time_zone(),0,deps.format_date);let table;try{table=runtime.table||deps.read_table();}catch(error){return subjectiveRecoveryEmptyProjection_("DATA_INTEGRITY_ERROR",today);}const loaded=subjectiveRecoveryReadEvents_(String(userId),table,now,deps);if(!loaded.ok)return subjectiveRecoveryEmptyProjection_("DATA_INTEGRITY_ERROR",today);if(!loaded.events.length)return subjectiveRecoveryEmptyProjection_("NO_DATA",today);const projection=subjectiveRecoveryEmptyProjection_("NORMAL",today),latest={};for(let i=0;i<loaded.events.length;i++){const event=loaded.events[i],time=new Date(event.reported_at).getTime();for(let m=0;m<SUBJECTIVE_RECOVERY_METRICS.length;m++){const key=SUBJECTIVE_RECOVERY_METRICS[m];if(subjectiveRecoveryBlank_(event[key]))continue;if(latest[key]&&latest[key].time===time)return subjectiveRecoveryEmptyProjection_("DATA_INTEGRITY_ERROR",today);if(!latest[key]||time>latest[key].time)latest[key]={time:time,event:event};}}SUBJECTIVE_RECOVERY_METRICS.forEach(function(key){if(!latest[key])return;const event=latest[key].event;projection.metrics[key]={status:event.local_date===today?"FRESH":"STALE",value:event[key]};if(event.local_date===today)projection.has_current_data=true;});return projection;}
function appendSubjectiveRecoveryCheckin_(userId,payload,options){const runtime=options||{},deps=subjectiveRecoveryDependencies_(runtime.dependencies),now=runtime.now instanceof Date?runtime.now:new Date(),reported=subjectiveRecoveryTimestamp_(payload&&payload.reported_at),event={schema_version:SUBJECTIVE_RECOVERY_SCHEMA_VERSION,event_id:String(payload&&payload.event_id||"").trim(),idempotency_key:String(payload&&payload.idempotency_key||"").trim(),user_id:String(userId||"").trim(),reported_at:reported?reported.toISOString():payload&&payload.reported_at,local_date:reported?nutritionCalendarDate_(reported,deps.time_zone(),0,deps.format_date):"",source:SUBJECTIVE_RECOVERY_SOURCE,created_at:now.toISOString()};SUBJECTIVE_RECOVERY_METRICS.forEach(function(key){event[key]=payload&&Object.prototype.hasOwnProperty.call(payload,key)?payload[key]:"";});const valid=validateSubjectiveRecoveryEvent_(event,now,deps);if(!valid.ok)return subjectiveRecoveryResult_(false,"INVALID_CHECKIN");const lock=deps.lock();if(!lock||!lock.tryLock(5000))return subjectiveRecoveryResult_(false,"LOCK_TIMEOUT");try{let table;try{table=deps.read_table();}catch(error){return subjectiveRecoveryResult_(false,"PERSISTENCE_FAILED");}const loaded=subjectiveRecoveryReadEvents_(event.user_id,table,now,deps);if(!loaded.ok)return subjectiveRecoveryResult_(false,"DATA_INTEGRITY_ERROR");for(let i=0;i<loaded.events.length;i++){const existing=loaded.events[i];if(existing.idempotency_key===event.idempotency_key){if(subjectiveRecoveryCanonical_(existing)===subjectiveRecoveryCanonical_(event))return subjectiveRecoveryResult_(true,"IDEMPOTENT_REPLAY",{event:existing,idempotent_replay:true});return subjectiveRecoveryResult_(false,"IDEMPOTENCY_CONFLICT");}if(existing.event_id===event.event_id)return subjectiveRecoveryResult_(false,"EVENT_ID_CONFLICT");}let row;try{row=deps.append_row(subjectiveRecoveryRecordValues_(event));}catch(error){return subjectiveRecoveryResult_(false,"PERSISTENCE_FAILED");}let readback;try{readback=deps.read_row(row);}catch(error){return subjectiveRecoveryResult_(false,"PERSISTENCE_READBACK_FAILED");}const durable=Array.isArray(readback)?subjectiveRecoveryValuesRecord_(readback):readback;if(!durable||!subjectiveRecoveryRecordsEqual_(durable,event,deps))return subjectiveRecoveryResult_(false,"PERSISTENCE_READBACK_FAILED");return subjectiveRecoveryResult_(true,"CHECKIN_APPENDED",{event:event,rows_written:1,verified:true});}finally{try{lock.releaseLock();}catch(ignored){}}}

/* SR-2 — deterministic Telegram capture and current-day presentation over SR-1. */
const SUBJECTIVE_RECOVERY_CAPTURE_SOURCE="SR2_SUBJECTIVE_RECOVERY";
function subjectiveRecoveryRouteResult_(handled,ok,code,extra){return Object.assign({handled:handled===true,ok:ok===true,code:String(code||""),groq_calls:0,recovery_writes:0,pending_capture_writes:0,production_writes:false},extra||{});}
function subjectiveRecoveryNormalize_(text){return String(text||"").toLowerCase().replace(/ё/g,"е").replace(/[!?]+/g,"").replace(/\s+/g," ").trim();}
function subjectiveRecoveryHasPain_(text){return /(?:болит|болят|боль|простреливает|тянет\s+(?:пах|сустав|колен|плеч|локт|кист|поясниц)|острая\s+боль)/.test(text);}
function subjectiveRecoverySetMetric_(metrics,key,raw){const value=Number(String(raw).replace(",","."));if(!isFinite(value))return false;metrics[key]=value;return true;}
function parseSubjectiveRecoveryInput_(text){
  const n=subjectiveRecoveryNormalize_(text),metrics={},pain=subjectiveRecoveryHasPain_(n),correction=/^(?:не\s+\d|исправь\b)/.test(n);
  if(!n)return {status:"NOT_RECOVERY",metrics:{},pain:false};
  if(correction&&/(?:устал|стресс|сон|самочувств|готовност|забит|болезненност)/.test(n))return {status:"CLARIFICATION",metrics:{},pain:pain,message:"Укажи новое значение, например: «усталость 6»."};
  if(/(?:^|[\s,;])сон\s*-?\d+(?:[.,]\d+)?(?:$|[\s,;.])/.test(n)&&!/качество\s+сна/.test(n)&&!/(?:час|ч\s|:)/.test(n))return {status:"AMBIGUOUS",metrics:{},pain:pain,message:"Уточни: сон 7 часов или качество сна 7/10?"};
  let match;
  const durationPatterns=[/спал(?:а)?\s*(-?\d+(?:[.,]\d+)?)\s*(?:час(?:а|ов)?|ч)(?:\s*(\d+)\s*минут(?:а|ы)?)?/,/сон\s*(-?\d+(?:[.,]\d+)?)\s*(?:час(?:а|ов)?|ч)/,/сон\s*(\d{1,2}):(\d{1,2})/];
  for(let i=0;i<durationPatterns.length&&!Object.prototype.hasOwnProperty.call(metrics,"sleep_duration_hours");i++){match=n.match(durationPatterns[i]);if(match){let hours=Number(String(match[1]).replace(",",".")),minutes=match[2]===undefined?0:Number(match[2]);if(minutes<0||minutes>=60)return {status:"INVALID",metrics:{},pain:pain};metrics.sleep_duration_hours=hours+minutes/60;}}
  const patterns=[["sleep_quality",/качество\s+сна\s*(?:[:=]\s*)?(-?\d+(?:[.,]\d+)?)/],["fatigue",/(?:усталость|устал(?:а)?\s+на)\s*(?:[:=]\s*)?(-?\d+(?:[.,]\d+)?)/],["stress",/стресс\s*(?:[:=]\s*)?(-?\d+(?:[.,]\d+)?)/],["wellbeing",/самочувствие\s*(?:[:=]\s*)?(-?\d+(?:[.,]\d+)?)/],["readiness",/готовность(?:\s+тренироваться)?\s*(?:[:=]\s*)?(-?\d+(?:[.,]\d+)?)/],["soreness",/(?:мышечная\s+болезненность|мышцы\s+забиты(?:\s+на)?|забитость\s+мышц)\s*(?:[:=]\s*)?(-?\d+(?:[.,]\d+)?)/]];
  patterns.forEach(function(item){const found=n.match(item[1]);if(found)subjectiveRecoverySetMetric_(metrics,item[0],found[1]);});
  const keys=Object.keys(metrics),recoveryWords=/(?:сон|спал|устал|стресс|самочувств|чувствую\s+себя|готовност|забит|мышечн|сил нет)/.test(n);
  if(!keys.length){if(pain)return {status:"PAIN_ONLY",metrics:{},pain:true};if(recoveryWords)return {status:"CLARIFICATION",metrics:{},pain:false,message:"Укажи числовое значение: например, «усталость 6» или «спал 7 часов»."};return {status:"NOT_RECOVERY",metrics:{},pain:false};}
  const valid=keys.every(function(key){const value=metrics[key];return isFinite(value)&&(key==="sleep_duration_hours"?value>=0&&value<=18:value>=0&&value<=10&&Math.floor(value)===value);});
  return {status:valid?"VALID":"INVALID",metrics:valid?metrics:{},pain:pain};
}
function subjectiveRecoveryMetricText_(key,value){const labels={sleep_quality:"качество сна",fatigue:"усталость",soreness:"мышечная болезненность",stress:"стресс",wellbeing:"самочувствие",readiness:"готовность"};if(key==="sleep_duration_hours")return "сон "+dailyNutritionNumber_(value,2)+" ч";return labels[key]+" "+dailyNutritionNumber_(value,0)+"/10";}
function formatSubjectiveRecoveryMetrics_(metrics){return SUBJECTIVE_RECOVERY_METRICS.filter(function(key){return Object.prototype.hasOwnProperty.call(metrics,key);}).map(function(key){return subjectiveRecoveryMetricText_(key,metrics[key]);});}
function detectSubjectiveRecoveryTodayIntent_(text){const n=subjectiveRecoveryNormalize_(text);if(!n)return null;return /^(?:как у меня восстановление сегодня|что по восстановлению сегодня|покажи восстановление|что я сегодня отмечал по восстановлению|мои показатели восстановления|покажи сон и усталость сегодня|сколько я спал сегодня)$/.test(n)?{intent:"RECOVERY_TODAY"}:null;}
function formatSubjectiveRecoveryToday_(projection){if(!projection||projection.data_status==="DATA_INTEGRITY_ERROR")return "Не удалось надёжно прочитать данные восстановления.";const labels={sleep_duration_hours:"Сон",sleep_quality:"Качество сна",fatigue:"Усталость",soreness:"Мышечная болезненность",stress:"Стресс",wellbeing:"Самочувствие",readiness:"Готовность"},lines=[];SUBJECTIVE_RECOVERY_METRICS.forEach(function(key){const metric=projection.metrics&&projection.metrics[key];if(!metric||metric.status!=="FRESH")return;lines.push(labels[key]+": "+(key==="sleep_duration_hours"?dailyNutritionNumber_(metric.value,2)+" ч":dailyNutritionNumber_(metric.value,0)+"/10"));});return lines.length?["Сегодня по восстановлению:"].concat(lines).join("\n"):"Сегодня данных по восстановлению пока нет.";}
function subjectiveRecoveryMessageDate_(message,now){const seconds=Number(message&&message.date);return isFinite(seconds)&&seconds>0?new Date(seconds*1000):new Date(now.getTime());}
function subjectiveRecoveryStableToken_(value){return String(value==null?"":value).replace(/[^A-Za-z0-9._-]/g,"").slice(0,120);}
function buildSubjectiveRecoveryWrite_(userId,metrics,identity,reportedAt){const token=subjectiveRecoveryStableToken_(identity);return Object.assign({event_id:"sr2-event-"+token,idempotency_key:"sr2-write-"+token,reported_at:reportedAt.toISOString()},metrics);}
function subjectiveRecoveryDependenciesSR2_(overrides){const supplied=overrides||{};return {detect_confirmation:supplied.detect_confirmation||detectConfirmationIntent_,find_capture:supplied.find_capture||subjectiveRecoveryFindActiveCapture_,get_pending:supplied.get_pending||getPendingCapture_,create_capture:supplied.create_capture||createPendingCapture_,cancel_capture:supplied.cancel_capture||cancelPendingCapture_,finalize_capture:supplied.finalize_capture||function(capture,result,now){smartConfirmationUpdateState_(smartConfirmationSheet_(),capture.row_number,"SAVED",JSON.stringify(result),now,"");SpreadsheetApp.flush();return {ok:true};},write:supplied.write||appendSubjectiveRecoveryCheckin_,read:supplied.read||loadAuthoritativeSubjectiveRecovery_};}
function routeSubjectiveRecovery_(update,options){
  const runtime=options||{},message=update&&(update.message||update.edited_message);if(!message||typeof message.text!=="string")return subjectiveRecoveryRouteResult_(false,true,"NOT_SUBJECTIVE_RECOVERY");
  const userId=String(message.from&&message.from.id||""),chatId=String(message.chat&&message.chat.id||"");if(!userId||!chatId)return subjectiveRecoveryRouteResult_(false,true,"NOT_SUBJECTIVE_RECOVERY");
  const now=runtime.now instanceof Date?runtime.now:new Date(),deps=subjectiveRecoveryDependenciesSR2_(runtime.dependencies),confirmation=deps.detect_confirmation(message.text);
  if(confirmation&&["CONFIRM","CANCEL"].indexOf(confirmation.intent)>=0){const selected=deps.find_capture(userId,chatId,{now:now,include_saved:confirmation.intent==="CONFIRM"});if(selected&&selected.ok){if(confirmation.intent==="CANCEL"){const cancelled=deps.cancel_capture(userId,chatId,{now:now});return subjectiveRecoveryRouteResult_(true,!!(cancelled&&cancelled.ok),cancelled&&cancelled.code||"CANCEL_FAILED",{message:cancelled&&cancelled.ok?"Запись отменена.":"Не удалось отменить запись."});}const frozen=selected.payload&&selected.payload.frozen||{},saved=deps.write(userId,buildSubjectiveRecoveryWrite_(userId,frozen.metrics||{},"capture-"+selected.payload.capture_id,new Date(frozen.reported_at)),{now:now});if(!saved||!saved.ok)return subjectiveRecoveryRouteResult_(true,false,saved&&saved.code||"PERSISTENCE_FAILED",{message:"Не удалось сохранить данные восстановления. Попробуйте ещё раз."});if(selected.capture.status!=="SAVED")deps.finalize_capture(selected.capture,{code:saved.code},now);return subjectiveRecoveryRouteResult_(true,true,saved.code,{recovery_writes:saved.rows_written||0,message:saved.idempotent_replay?"Эти данные восстановления уже сохранены.":"Записал данные восстановления."});}if(selected&&selected.code==="CAPTURE_EXPIRED")return subjectiveRecoveryRouteResult_(true,false,"CAPTURE_EXPIRED",{message:"Срок подтверждения истёк. Отправьте данные ещё раз."});}
  const todayIntent=detectSubjectiveRecoveryTodayIntent_(message.text);if(todayIntent){const projection=deps.read(userId,{now:now,dependencies:runtime.reader_dependencies});return subjectiveRecoveryRouteResult_(true,projection&&projection.data_status!=="DATA_INTEGRITY_ERROR",projection&&projection.data_status||"DATA_INTEGRITY_ERROR",{message:formatSubjectiveRecoveryToday_(projection),projection:projection});}
  const parsed=parseSubjectiveRecoveryInput_(message.text);if(parsed.status==="NOT_RECOVERY")return subjectiveRecoveryRouteResult_(false,true,"NOT_SUBJECTIVE_RECOVERY");if(parsed.status==="PAIN_ONLY")return subjectiveRecoveryRouteResult_(true,true,"PAIN_NOT_SORENESS",{message:"Боль не записываю как мышечную болезненность. Для восстановления можно указать забитость/мышечную болезненность по шкале 0–10."});if(parsed.status==="AMBIGUOUS"||parsed.status==="CLARIFICATION")return subjectiveRecoveryRouteResult_(true,false,"CLARIFICATION_REQUIRED",{message:parsed.message});if(parsed.status==="INVALID")return subjectiveRecoveryRouteResult_(true,false,"INVALID_VALUE",{message:"Значение вне допустимого диапазона. Ничего не записано."});
  const names=formatSubjectiveRecoveryMetrics_(parsed.metrics),reported=subjectiveRecoveryMessageDate_(message,now),painNote=parsed.pain?" Боль не записана как мышечная болезненность.":"";
  if(names.length===1){const identity="update-"+subjectiveRecoveryStableToken_(update&&update.update_id)+"-user-"+subjectiveRecoveryStableToken_(userId),saved=deps.write(userId,buildSubjectiveRecoveryWrite_(userId,parsed.metrics,identity,reported),{now:now});return subjectiveRecoveryRouteResult_(true,!!(saved&&saved.ok),saved&&saved.code||"PERSISTENCE_FAILED",{recovery_writes:saved&&saved.rows_written||0,message:saved&&saved.ok?"Записал: "+names[0]+"."+painNote:"Не удалось сохранить данные восстановления. Попробуйте ещё раз."});}
  const active=deps.get_pending(userId,chatId,{now:now});if(active&&active.ok)return subjectiveRecoveryRouteResult_(true,false,"ACTIVE_CAPTURE_EXISTS",{message:"Сначала завершите или отмените текущее подтверждение данных."});const captureId="sr2-"+subjectiveRecoveryStableToken_(update&&update.update_id),capture={schema_version:"sr2-subjective-recovery-capture-v1",source:SUBJECTIVE_RECOVERY_CAPTURE_SOURCE,capture_id:captureId,user_id:userId,chat_id:chatId,raw_message:"",items:[{category:"RECOVERY_LOG",fields:{}}],frozen:{metrics:Object.assign({},parsed.metrics),reported_at:reported.toISOString()}};const created=deps.create_capture(capture,{now:now,ttl_minutes:30,capture_id:captureId,user_id:userId,chat_id:chatId,source_update_id:update&&update.update_id,validation:{ready_for_confirmation:true,errors:[]}});return subjectiveRecoveryRouteResult_(true,!!(created&&created.ok),created&&created.code||"CAPTURE_CREATE_FAILED",{pending_capture_writes:created&&created.created===false?0:created&&created.ok?1:0,message:created&&created.ok?"Записать: "+names.join(", ")+"?"+painNote:"Не удалось подготовить подтверждение."});
}

/* Terminal Recovery captures never own a later generic confirmation. */
function subjectiveRecoveryFindActiveCapture_(userId,chatId,options){const now=options&&options.now instanceof Date?options.now:new Date();try{const rows=smartConfirmationReadRows_(smartConfirmationSheet_()).filter(function(row){const payload=smartConfirmationParseJson_(row.payload_json,{});return String(row.user_id)===String(userId)&&String(row.chat_id)===String(chatId)&&payload.source===SUBJECTIVE_RECOVERY_CAPTURE_SOURCE&&row.status==="PENDING_CONFIRMATION"&&new Date(row.expires_at).getTime()>now.getTime();}).sort(smartConfirmationNewestFirst_),pending=rows[0];return pending?{ok:true,code:"PENDING_CAPTURE",capture:pending,payload:smartConfirmationParseJson_(pending.payload_json,{})}:{ok:false,code:"NO_RECOVERY_CAPTURE"};}catch(error){return {ok:false,code:"CAPTURE_LOOKUP_FAILED"};}}
...
function domainFactDependencies_(injected) {
  if (injected) {
    return Object.assign({
      ...
      build_dashboard: buildDailyDashboardFacts_,
      evaluate_contextual: evaluateContextualCoachingHint_
    }, injected);
  }
  return {
