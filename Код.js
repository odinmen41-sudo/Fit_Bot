...
function subjectiveRecoveryRecordsEqual_(actual,expected){return SUBJECTIVE_RECOVERY_SCHEMA.every(function(header){const key=header.toLowerCase();return String(actual&&actual[key])===String(expected&&expected[key]);});}
function validateSubjectiveRecoveryEvent_(event,now,deps){if(!event||String(event.schema_version)!==SUBJECTIVE_RECOVERY_SCHEMA_VERSION||!String(event.event_id||"").trim()||!String(event.idempotency_key||"").trim()||!String(event.user_id||"").trim()||String(event.source)!==SUBJECTIVE_RECOVERY_SOURCE)return {ok:false};const reported=subjectiveRecoveryTimestamp_(event.reported_at),created=subjectiveRecoveryTimestamp_(event.created_at);if(!reported||!created||reported.getTime()>now.getTime()||created.getTime()>now.getTime()||String(event.local_date)!==nutritionCalendarDate_(reported,deps.time_zone(),0,deps.format_date))return {ok:false};let count=0;for(let i=0;i<SUBJECTIVE_RECOVERY_METRICS.length;i++){const key=SUBJECTIVE_RECOVERY_METRICS[i],value=event[key];if(subjectiveRecoveryBlank_(value))continue;count++;const number=Number(value);if(!isFinite(number))return {ok:false};if(key==="sleep_duration_hours"){if(number<0||number>18)return {ok:false};}else if(number<0||number>10||Math.floor(number)!==number)return {ok:false};event[key]=number;}if(!count)return {ok:false};event.reported_at=reported.toISOString();event.created_at=created.toISOString();return {ok:true,event:event};}
