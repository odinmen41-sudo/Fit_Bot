...
function readWorkoutLifecycleTable_(){const sheet=getSpreadsheet_().getSheetByName("Workout_Log");if(!sheet)return null;const cols=Math.min(sheet.getLastColumn(),WORKOUT_LOG_SCHEMA.length),rows=sheet.getLastRow();if(!rows)return null;const values=sheet.getRange(1,1,rows,cols).getValues();return {headers:values[0],rows:values.slice(1)};}
function appendWorkoutLifecycleRows_(rows){const lock=LockService.getScriptLock();if(!lock.tryLock(5000))return {ok:false,code:"LOCK_TIMEOUT"};try{const sheet=getSpreadsheet_().getSheetByName("Workout_Log");if(!sheet)return {ok:false,code:"WORKOUT_LOG_NOT_CONFIGURED"};const headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getDisplayValues()[0],schema=validateWorkoutLogSchema_({headers:headers,rows:[]});if(!schema.ok)return {ok:false,code:"SCHEMA_UPGRADE_REQUIRED"};const existing=sheet.getLastRow()>1?sheet.getRange(2,schema.indexes.IDEMPOTENCY_KEY+1,sheet.getLastRow()-1,1).getDisplayValues().map(function(r){return String(r[0]);}):[],keys=rows.map(function(r){return String(r[schema.indexes.IDEMPOTENCY_KEY]);});if(keys.some(function(k){return existing.indexOf(k)>=0;}))return {ok:true,code:"IDEMPOTENT_REPLAY",rows_written:0,idempotent_replay:true};if(keys.some(function(k,i){return !k||keys.indexOf(k)!==i;}))return {ok:false,code:"INVALID_IDEMPOTENCY_BATCH"};if(rows.length)sheet.getRange(sheet.getLastRow()+1,1,rows.length,WORKOUT_LOG_SCHEMA.length).setValues(rows);SpreadsheetApp.flush();return {ok:true,code:"APPENDED",rows_written:rows.length};}finally{lock.releaseLock();}}
...
function workoutAppend_(deps,rows){return deps.append_atomic(rows);}
