...
function loadEffectiveWorkoutHistory_(telegramUserId,options){
  ...
  for(let i=0;i<events.length;i+=1){const e=events[i],sid=String(e.SESSION_ID),eid=String(e.EVENT_ID);if(eventIds[eid])return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});eventIds[eid]=true;
    if(!sessions[sid])sessions[sid]={session_id:sid,mode:String(e.SESSION_MODE||""),planned_date:String(e.PLANNED_DATE||""),session_key:String(e.SESSION_KEY||""),display_name:String(e.SESSION_NAME||"Тренировка"),status:"",started_at:null,completed_at:null,sets:[],skipped:[],events:[]};
    const s=sessions[sid],type=String(e.EVENT_TYPE);if(s.mode!==String(e.SESSION_MODE||"")||s.planned_date!==String(e.PLANNED_DATE||"")||s.session_key!==String(e.SESSION_KEY||"")||s.display_name!==String(e.SESSION_NAME||"Тренировка"))return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});if(type!=="SESSION_START"&&!s.started_at||s.completed_at)return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});sessions[sid].events.push(e);if(type==="SESSION_START"){const at=workoutValidTimestamp_(e.PERFORMED_AT);if(s.started_at||!at||!["PLAN_LINKED","AD_HOC"].includes(s.mode))return workoutActualResult_(true,false,"DATA_INTEGRITY_ERROR",{sessions:[]});s.started_at=at;s.status="IN_PROGRESS";}
